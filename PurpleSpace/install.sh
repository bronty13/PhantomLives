#!/bin/bash
# Reinstall the freshly-built Purple Space.app into /Applications/.
#
# Usage:
...
# Also reap any embedded Convex backend orphaned by the force-kill above —
# the relaunched app must spawn the backend from ITS bundle, never adopt a
# stale-binary orphan across an upgrade. (Identified by its sqlite path
# argument under our Application Support dir.)
pkill -f "convex-local-backend.*Application Support/Purple Space" 2>/dev/null || true
for _ in $(seq 1 25); do
    pgrep -f "convex-local-backend.*Application Support/Purple Space" >/dev/null 2>&1 || break
    sleep 0.2
done
