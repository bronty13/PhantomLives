 ...
 *   - zips the entire Application Support dir (prefs + the embedded Convex
 *     SQLite database) on launch
 ...
 *
 * The launch backup runs BEFORE the embedded Convex backend starts, so the
 * SQLite file is zipped while quiescent (no mid-write inconsistency).
 *
 * Default location: ~/Downloads/Purple Space backup/
 * Filename:         Purple Space-YYYY-MM-DD-HHmmss.zip
 */
...
import type { BackupInfo, BackupRunResult, BackupTestResult } from '../../shared/types';
import { getPreferences, setPreferences } from '../prefs';

const PREFIX = (): string => `${app.getName()}-`;
...
export async function runBackup(force = false): Promise<BackupRunResult> {
  try {
    ...
    // Log, never throw. The app must still launch.
    console.error('[PurpleSpace] backup failed:', msg);
...
export async function testBackup(path: string): Promise<BackupTestResult> {
  try {
    ...
    const hasPrefs = files.some((f) => f.endsWith('purple-space-prefs.json'));
...
export async function restoreBackup(path: string): Promise<BackupRunResult> {
  try {
    ...
    const zip = await JSZip.loadAsync(await readFile(path));
    const staging = join(tmpdir(), `purplespace-restore-${stamp()}`);
