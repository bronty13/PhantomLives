...
const h = vi.hoisted(() => {
  ...
  const base = fs.mkdtempSync(path.join(os.tmpdir(), 'ps-backup-'));
  const dataDir = path.join(base, 'data');
  const backupDir = path.join(base, 'backups');
  fs.mkdirSync(dataDir, { recursive: true });
  fs.writeFileSync(path.join(dataDir, 'purple-space-prefs.json'), '{"version":1}');
  // Simulate the embedded Convex data dir living inside App Support.
  fs.mkdirSync(path.join(dataDir, 'convex'), { recursive: true });
  fs.writeFileSync(path.join(dataDir, 'convex', 'convex_local_backend.sqlite3'), 'sqlite-bytes');
  const prefs = {
...
vi.mock('electron', () => ({
  app: { getName: () => 'Purple Space', getPath: () => h.dataDir }
}));
...
import { runBackup, listBackups, trimRetention, testBackup } from '../../src/main/backup/backupService';
...
describe('BackupService', () => {
  it('creates a backup zip and auto-creates the destination dir', async () => {
    ...
    expect(r.info?.name).toMatch(/^Purple Space-/);
    expect(existsSync(r.info!.path)).toBe(true);
    expect(existsSync(h.backupDir)).toBe(true);
  });

  it('includes the Convex sqlite file and prefs in the archive', async () => {
    const r = await runBackup(true);
    const t = await testBackup(r.info!.path);
    expect(t.ok).toBe(true);
    expect(t.hasPrefs).toBe(true);
    expect(t.fileCount).toBeGreaterThanOrEqual(2);
  });
  ...
  it('skips when auto-backup is disabled (non-forced)', async () => {
    h.prefs.autoBackupEnabled = false;
    const r = await runBackup(false);
    expect(r.skipped).toBe(true);
  });

  it('lists backups newest-first', async () => {
    const a = join(h.backupDir, 'Purple Space-2026-01-01-000000.zip');
    const b = join(h.backupDir, 'Purple Space-2026-06-01-000000.zip');
    writeFileSync(a, 'x');
    writeFileSync(b, 'x');
  ...
  it('only trims our-prefixed files older than the retention window', async () => {
    const ours = join(h.backupDir, 'Purple Space-2020-01-01-000000.zip');
