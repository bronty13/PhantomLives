...
- **Photo & video attachments.** Media you attach to an entry — photos imported
  from Apple Photos or from Files, and videos added from Files — is stored as
  BLOBs *inside* `diary.sqlite` (photos as a downscaled JPEG plus a thumbnail;
  videos byte-for-byte plus a poster-frame thumbnail), so it inherits the same
  SQLCipher encryption at rest as your text — there are no separate plaintext
  media files on disk, and it rides along inside the encrypted backup zip. (One
  practical consequence: an uncompressed video lives in the database, so a large
  video grows both the DB and every launch backup.)
...
|---|---|---|
| `diary.sqlite` (entire file) | **SQLCipher 4.6.1** | AES-256-CBC page encryption + HMAC-SHA512 per-page authentication. Vendored amalgamation, CommonCrypto backend. Every entry, the schema, all indexes, **and imported photo/video attachments (stored as BLOBs)** are opaque ciphertext without the DEK. |
...
- **Photo & video attachments are in the database.** Media you attach (photos
  from Apple Photos or Files, videos from Files) is stored as BLOBs inside the
  SQLCipher database (see §1, §3), so it's encrypted at rest and captured by
  backups like everything else. Photos are downscaled; **videos are stored
  uncompressed**, so a large video noticeably grows the database and every launch
  backup. Audio and arbitrary-file attachments are not yet supported.
...
*Drafted 2026-05-30. Last reviewed 2026-05-31 (photo & video attachments stored as encrypted BLOBs).*
