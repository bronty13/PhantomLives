// Auto-backup-on-launch — PhantomLives standard, ported from SideMolly's
// backup.rs (which itself ports Molly's / Timeliner's BackupService).
// Convention: every app that owns persistent user data zips its app-data
// directory on launch into `~/Downloads/<App> backup/`, retains for 14 days
// by default, debounces within 5 minutes, never panics, and exposes the
// Settings UI required by CLAUDE.md.
//
// PurpleMind-specific values:
//   - ARCHIVE_PREFIX = "PurpleMind-"
//   - APP_NAME       = "PurpleMind"
//   - Verify checks for `purplemind.db` inside the archive
...
pub struct Settings {
    ...
    /// Absolute path; if empty/missing, use the default `~/Downloads/PurpleMind backup/`.
...
pub(crate) fn run_backup(support_dir: &Path, backup_dir: &Path) -> Result<PathBuf, BackupError> {
...

    {
        ...
        // traversable (Finder otherwise reports "you don't have permission
        // to see its contents").
...
/// files whose name starts with `PurpleMind-` and ends `.zip` — never nukes
...
pub fn restore_archive(archive_path: &Path, support_dir: &Path) -> Result<PathBuf, BackupError> {
    ...
    if support_dir.exists() {
        if let Err(err) = run_backup(support_dir, &parent) {
            eprintln!("[purplemind] safety backup failed (continuing): {err}");
        } else {
...
mod tests {
    ...
    fn retention_trims_only_prefixed_zips() {
        ...
        let mine_old = dir.path().join("PurpleMind-2020-01-01-000000.zip");
        let mine_new = dir.path().join("PurpleMind-2099-01-01-000000.zip");
        ...
        assert_eq!(removed, 1, "exactly one PurpleMind-prefixed old archive removed");
        assert!(!mine_old.exists(), "old PurpleMind zip is gone");
        assert!(mine_new.exists(), "new PurpleMind zip kept");
    ...
    fn retention_zero_means_keep_forever() {
        let dir = TempDir::new().unwrap();
        let p = dir.path().join("PurpleMind-2020-01-01-000000.zip");
        fs::write(&p, b"x").unwrap();
        touch_age(&p, 10_000);
    ...
    fn run_backup_creates_target_dir_when_missing() {
        ...
        fs::write(support.path().join("backup-settings.json"), b"{}").unwrap();

        let backup_root = TempDir::new().unwrap();
        let backup_dir = backup_root.path().join("nested/PurpleMind backup");
        ...
        assert!(verify.has_database, "archive contains purplemind.db");
    ...
    fn list_returns_newest_first() {
        let dir = TempDir::new().unwrap();
        let a = dir.path().join("PurpleMind-2024-01-01-000000.zip");
        let b = dir.path().join("PurpleMind-2024-06-01-000000.zip");
        let c = dir.path().join("PurpleMind-2024-12-01-000000.zip");
