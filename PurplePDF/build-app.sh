#!/bin/bash
# Build Purple PDF.app for the host architecture and (by default) install
...
#
# For a signed universal2 release DMG, use `scripts/build-app.sh` instead
# (or `npm run dist:mac`). This root script is the PhantomLives-convention
# fast-iteration path: host-arch only, no DMG, no signing dance.
set -euo pipefail
...
# Auto-install: replace /Applications/Purple PDF.app and relaunch. Opt
