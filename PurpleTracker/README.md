...
- **File-store paths** — primary defaults to `~/Library/CloudStorage/OneDrive-defiSOLUTIONS/{year}/{YYYY-MM-DD} {Title}`, secondary to `~/Downloads/{Title}`. "Create" mkdir-p's; "Reveal" opens in Finder.
- **Exports** — Markdown, PDF, and Word `.docx`, plus copy-to-clipboard. "Copy Brief" puts `Matter ID • Title • Date Opened • Status` on the pasteboard.
