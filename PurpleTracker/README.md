...
- **Third Parties (1.4.0)** — separate sidebar section for tracking vendors / 3rd-party suppliers. Demographics, Sales/Escalation/Technical contacts, repeatable products, 1–5 star rating, reseller (Cyber One / CDW / Other), data center, exit strategy, contract & costing summaries. Per-vendor **Budget vs Actuals** matrix over a configurable year range (default 2026–2035); Actuals roll up from logged Invoices unless a manual override is set (explicit 0 wins). Contracts / Invoices / Notes / Other files stored as BLOB attachments. Timestamped notes with attachments. Matters can be linked to a Vendor. Single-vendor and all-vendors reports in Markdown + PDF.
- **Notes Workspace (1.5.0)** — top-level **Notes** section in the sidebar with user-configurable Note Types (defaults: Staff, Architecture, Team, SCRUM, Third Party). Each note has a date, title, and a full **WYSIWYG rich-text editor** with bold / italic / underline / strikethrough, headings, bullet & numbered lists, hyperlinks, color, clear-formatting, and standard keyboard shortcuts (⌘B / ⌘I / ⌘U / ⇧⌘X, ⇧⌘7-8, ⌘K, ⌥⌘1-3 / ⌥⌘0). Pasted screenshots and images are persisted in the body (RTFD). Manage Note Types in Settings → Note Types.
