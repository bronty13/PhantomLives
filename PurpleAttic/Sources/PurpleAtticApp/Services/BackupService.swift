...
/// Auto-backup-on-launch for PurpleAttic (PhantomLives standard). Zips the app's support
/// directory — `~/Library/Application Support/PurpleAttic/` (profiles + settings; the things
/// a user can't trivially recreate) — into `~/Downloads/PurpleAttic backup/`. 14-day
/// retention, 5-minute debounce, never throws on the launch path.
///
/// NB: the photo *archive itself* is not backed up here — it's hundreds of GB on external
/// disks with its own 3-copy strategy. This protects the app's configuration/state only.
enum BackupService {

    static let debounceSeconds: TimeInterval = 5 * 60
    static let archivePrefix = "PurpleAttic-"
    ...
    static func supportDirectory() -> URL { ProfileStore.defaultDirectory() }

    static func runOnLaunchIfDue(settingsStore: SettingsStore) {
        guard settingsStore.settings.autoBackupEnabled else { return }
        if let raw = settingsStore.settings.lastBackupAt,
           let last = parseISO(raw),
           Date().timeIntervalSince(last) < debounceSeconds {
            return
        }
        do {
            let url = try doBackup(settingsStore: settingsStore)
            NSLog("PurpleAttic: backup-on-launch wrote \(url.lastPathComponent)")
        } catch {
            NSLog("PurpleAttic: backup-on-launch failed — \(error.localizedDescription)")
        }
    }
    ...
    static func doBackup(settingsStore: SettingsStore) throws -> URL {
        ...
        let url = try runBackup(supportDir: support, backupDir: backupDir)
    ...
    static func runBackup(supportDir: URL, backupDir: URL) throws -> URL {
        ...
        let tempZip = fm.temporaryDirectory
            .appendingPathComponent("purpleattic-backup-\(UUID().uuidString).zip")
