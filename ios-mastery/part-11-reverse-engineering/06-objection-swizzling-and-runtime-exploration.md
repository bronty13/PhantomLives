...
On a non-jailbroken device — the realistic 2026 case, since there is **no public kernel jailbreak for A12+ on iOS 18/26** ([[07-the-jailbreak-landscape-2026]]) — objection's headline feature is **`objection patchipa`**: it automates the entire FridaGadget-embedding pipeline ([[04-the-app-bundle-and-ipa-structure]], OWASP **MASTG-TECH-0090**) that you'd otherwise do by hand:
...
You then install the resulting `MyApp-frida-codesigned.ipa` (Xcode, `ios-deploy`, Sideloadly, or — for the no-cert path — a CoreTrust-class loader like TrollStore on ≤ iOS 17.0, see [[08-trollstore-and-the-coretrust-bug]]), launch it, and connect with `objection explore`. At launch, `dyld` honors the new `LC_LOAD_DYLIB`, maps `FridaGadget.dylib` into the app's *own* sandbox, and the gadget opens a Frida server **inside the process** — no foreign task port, no `frida-server`, no jailbreak. The cost is that you need a re-signable IPA and a signing identity, and you only instrument *that one app*.
...
| **Filesystem** | `ls` · `cd` · `pwd print` · `file download <r> <l>` · `file upload <l> <r>` · `file cat <r>` | Walk and exfiltrate the app's sandbox container ([[00-app-sandbox-and-filesystem-layout]]) |
...
The two you'll reach for first are `env` (to learn the container layout for filesystem triage) and `ios hooking search classes <keyword>` (to find the class that gates the behavior you care about, without opening a disassembler). The `import` command is the escape hatch: when objection's pre-built hook doesn't fit, you drop to a hand-written agent — back to [[05-dynamic-analysis-with-frida]] — inside the same session.
...
The lesson for both sides: this only works because the app trusted a boolean. The *correct* pattern binds biometrics to a key in the Secure Enclave-backed keychain via `kSecAccessControlBiometryCurrentSet` / `SecAccessControlCreateWithFlags`, so the protected secret is *only decryptable* after a real biometric match ([[08-keychain-on-ios]], [[07-biometrics-security-architecture]]) — there's no boolean to flip, because the gate is the SEP releasing a key, not app code branching. A builder who reads this lesson designs the second way; an RE who reads it knows the first way is one objection command away. (Caveat: on the **Simulator** there's no SEP and biometrics are an enrollment toggle in the Features menu, so this bypass is a *runtime-mechanics* demo, not a real defeat.)
...
Because swizzling is "rewrite the IMP slot," an app can detect it by **inspecting its own dispatch tables** — the both-sides game of [[11-anti-tamper-pinning-and-detection-both-sides]]. The standard checks:
...
- It saw meaningful development only ~2009–2013; cynject/Substrate injection broke around **iOS 12 (~2019)**, and **PAC** (A12+, [[06-kernel-hardening-pac-sptm-txm-mie]]) plus library validation break its remaining workflows on iOS 15+.
...
> 🔬 **Forensics note:** `memory dump all` over a live process captures decrypted strings, keys, and buffers that never hit disk — the runtime cousin of a FairPlay in-memory dump ([[03-fairplay-encryption-and-decrypting-app-store-apps]]). Carve it with `strings`/`rabin2 -z`, but record that it's a *live* artifact: hash the dump, note the bundle UUID and the exact `memory list modules` slide so addresses are reconstructable, and keep the objection command transcript alongside it.
...
3. Run `env` — record the four container paths. Cross-check one against the real on-disk location under `~/Library/Developer/CoreSimulator/Devices/<UDID>/data/Containers/` ([[01-simulator-internals-and-on-disk-filesystem]]).
...
4. Run `ios keychain dump`. Observe it's empty/sparse — write one sentence on *why* (the Simulator has no SEP-backed keychain / Data-Protection classes; [[08-keychain-on-ios]], [[02-data-protection-and-keybags]]).
...
2. Point the app's traffic at a proxy (mitmproxy/Burp) with its CA trusted by the Simulator; confirm the request **fails** while pinning is on ([[02-traffic-interception-and-tls]], [[03-certificate-pinning-and-bypass]]).
...
1. Obtain an authorized IPA (extracted/decrypted, [[03-fairplay-encryption-and-decrypting-app-store-apps]]).
...
- **Pure-Swift is as hard for objection as for Frida.** `ios hooking list classes` enumerates the **Objective-C** runtime. A pure-Swift type with no `@objc`/`NSObject` base won't appear, and `set return_value` can't target it — you need the Swift bridge / mangled symbols, exactly as in [[05-dynamic-analysis-with-frida]]. Many "Swift" apps are ObjC-bridged where it counts; check first.
...
- **`patchipa` changes the binary — anti-tamper can see it.** Adding `LC_LOAD_DYLIB` and re-signing alters the Mach-O and the Team ID; integrity checks fire ([[11-anti-tamper-pinning-and-detection-both-sides]]). And a 7-day free cert means the patched app dies in a week.
...
- **OWASP MASTG/MASVS** — *MASTG-TOOL-0038: objection*; *MASTG-TECH-0090: Injecting Frida Gadget into an IPA*; *MASTG-TECH-0061: Dumping KeyChain Data*; *MASTG-TOOL-0046: Cycript*, *MASTG-TOOL-0049: Frida-cycript* ([[10-owasp-mastg-and-app-security-testing]]).
- **Frida** — frida.re *JavaScript API* (ObjC bridge, `Interceptor`); `frida-objc-bridge` repo (`implement`, the IMP machinery) ([[05-dynamic-analysis-with-frida]]).
...
- **Pinning & tweaks (adjacent)** — TrustKit, AFNetworking `AFSecurityPolicy`; Theos/ElleKit/Logos `%hook` as device-side swizzling ([[09-tweak-development-with-theos]], [[03-certificate-pinning-and-bypass]]).
...
---
*Related lessons: [[05-dynamic-analysis-with-frida]] | [[04-static-analysis-class-dump-and-disassemblers]] | [[03-certificate-pinning-and-bypass]] | [[11-anti-tamper-pinning-and-detection-both-sides]] | [[09-tweak-development-with-theos]] | [[03-fairplay-encryption-and-decrypting-app-store-apps]] | [[06-code-signing-and-provisioning-in-depth]] | [[10-owasp-mastg-and-app-security-testing]]*
