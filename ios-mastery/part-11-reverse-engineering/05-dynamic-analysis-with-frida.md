...
- **frida-gum** is the C engine: the actual machinery that rewrites a function prologue, JIT-recompiles a basic block, reads/writes memory, walks modules. Platform-specific, ARM64/ARM64e-aware (it handles PAC signing of patched pointers — see [[06-kernel-hardening-pac-sptm-txm-mie]]).
...
- **frida-tools** is the Python CLI layer you actually type: `frida` (REPL), `frida-trace`, `frida-ps`, `frida-ls-devices`, `frida-kill`, plus the third-party `objection` ([[06-objection-swizzling-and-runtime-exploration]]).
...
> 🖥️ **macOS contrast:** You've used this exact engine on macOS — `frida -n Safari`, or `frida-trace -m '-[NSURLSession dataTaskWithRequest:*]' Safari`. The injection primitive there is the Mach `task_for_pid` → `mach_vm_allocate` → `thread_create_running` dance you met in [[05-processes-mach-xpc]]: Frida grabs the target's **task port**, maps in the agent, and spins up a thread to run it. On macOS this works for your own processes but is gated for SIP-protected / hardened-runtime binaries (you need the `com.apple.security.cs.debugger` entitlement or SIP disabled). `dtrace`/`dtruss` is the kernel-level cousin — system-wide, probe-based, but read-mostly and itself SIP-restricted. The thing to internalize: **on an iOS *device* none of this is available to you** — the kernel denies you the task port (no `task_for_pid` for unprivileged code), AMFI refuses to execute the unsigned agent, and the sandbox blocks `ptrace`. That's why the device needs either a jailbreak (to run a privileged `frida-server`) or a *gadget* baked into the app. The Simulator, by contrast, is a native macOS process you own — so Frida injects exactly as it does into Safari.

### How the agent gets in: injection internals

It's worth understanding the actual mechanism, because it's *exactly* what AMFI/sandbox block on a device and *exactly* what you have for free in the Simulator. When you `frida -p <pid>`, frida-core performs a classic **Mach injection** (the technique you traced in [[05-processes-mach-xpc]]):
...
The practical division of labor: **static** to find the address/method, **Frida** to watch and rewrite it at scale, **lldb** to single-step the one gnarly function Frida flagged. Frida and lldb coexist — you can `Interceptor.attach` to log every call site and drop an lldb breakpoint on the one that's interesting ([[11-debugging-instruments-and-lldb-for-ios]]).

### Frida detection — the cat-and-mouse

Hardened apps actively hunt for Frida, and as an RE you'll both *trip* and *defeat* these checks. The common tells (all of which you then neutralize with the very hooks above — this is the both-sides game of [[11-anti-tamper-pinning-and-detection-both-sides]]):
...
4. **Deliverable:** before/after behavior, and a note on the difference between `Interceptor.attach` (observe, then optionally `retval.replace`) and reassigning `.implementation` (full swizzle). This is the conceptual core of [[06-objection-swizzling-and-runtime-exploration]].
...
1. Jailbreak a compatible device (**A8–A11 / checkm8, iOS ≤ 18.7.x, palera1n**; there is no public kernel jailbreak for A12+) — [[07-the-jailbreak-landscape-2026]].
...
- **`Interceptor` patches the prologue — anti-tamper can see it.** Apps that checksum their own `__TEXT` will notice the trampoline. Prefer `Stalker` (no prologue patch) or hook *above* the integrity check, and read [[11-anti-tamper-pinning-and-detection-both-sides]] before you assume a hook is invisible.
...
- **OWASP MASTG/MASVS** — *MASTG-TOOL-0031: Frida*; *MASTG-TECH-0090: Injecting Frida Gadget into an IPA Automatically*; the iGoat-Swift / UnCrackable crackmes ([[10-owasp-mastg-and-app-security-testing]]).
- **objection** — github.com/sensepost/objection (`patchipa`, jailbroken/jailed wikis); the Frida-built runtime toolkit ([[06-objection-swizzling-and-runtime-exploration]]).
- **frida-ios-dump / bagbak** — FairPlay in-memory dump via Frida ([[03-fairplay-encryption-and-decrypting-app-store-apps]]).
...
---
*Related lessons: [[04-static-analysis-class-dump-and-disassemblers]] | [[06-objection-swizzling-and-runtime-exploration]] | [[03-fairplay-encryption-and-decrypting-app-store-apps]] | [[03-certificate-pinning-and-bypass]] | [[11-anti-tamper-pinning-and-detection-both-sides]] | [[01-simulator-internals-and-on-disk-filesystem]] | [[07-the-jailbreak-landscape-2026]]*
