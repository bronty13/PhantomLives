...
4. `otool -l "$APP" | grep -A4 LC_MAIN` — take `entryoff`, add `__TEXT.vmaddr`, and you have the VA of `main()`. Sanity-check against `nm "$APP" | grep -i ' _main$'` (Swift `main` may be mangled — that's fine, you'll meet mangling in [[04-static-analysis-class-dump-and-disassemblers]]).
...
3. Trace the decryption path you'd use *with authorization* on a jailbroken device (full detail in [[03-fairplay-encryption-and-decrypting-app-store-apps]]): run the app → the OS's FairPlay path (`fairplayd` + the `FairPlayIOKit` kernel driver, via `mremap_encrypted`) decrypts `__TEXT` into memory → a tool like `frida-ios-dump`/`bagbak` reads the plaintext pages back, overwrites the `cryptoff` range, and sets `cryptid = 0` → you now have a static-analyzable Mach-O.
...
- **System dylib paths don't exist on disk.** `otool -L` shows `/System/Library/Frameworks/Foundation.framework/Foundation`, but on a device that file isn't there — it's in the dyld shared cache. To analyze it you extract from the cache (see [[02-the-dyld-shared-cache]]), you don't `find` it.
...
---
*Related lessons: [[04-the-app-bundle-and-ipa-structure]] | [[07-dyld-shared-cache-and-amfi]] | [[02-the-dyld-shared-cache]] | [[01-the-code-signature-blob-and-entitlements-on-ios]] | [[03-fairplay-encryption-and-decrypting-app-store-apps]] | [[04-static-analysis-class-dump-and-disassemblers]] | [[07-frameworks-dylibs-and-dynamic-linking]] | [[06-kernel-hardening-pac-sptm-txm-mie]]*
