...
- **libimobiledevice** (C, via Homebrew) — the classic suite. `idevice_id -l` (list attached UDIDs), `ideviceinfo` (lockdownd property dump), `idevicebackup2` (drive the *same* backup protocol Finder uses → an iTunes/Finder-format backup, see [[the-itunes-finder-backup-format]]), `ideviceinstaller` (separate formula; list/install/uninstall apps), `idevicesyslog` (live console stream), `idevicecrashreport` (pull crash logs). This is the spine of [[logical-acquisition-with-libimobiledevice]].
...
- **APOLLO** — *Apple Pattern of Life Lazy Output'er* (Sarah Edwards, `mac4n6/APOLLO`). The same tool you met on macOS, pointed at the iOS pattern-of-life stores: `knowledgeC.db`, `interactionC.db`, `CurrentPowerlog.PLSQL`, and Biome. Git clone, no pip; it runs its bundled SQL modules against a copy of a database and unifies them into one timeline. Pairs with [[knowledgec-db-deep-dive]].
...
- **img4tool** (tihmstar) / **ipsw img4** — unwrap and inspect **IMG4** containers (the `IM4P` payload, `IM4M` manifest/SHSH, `IM4R` restore-info). Central to [[image4-personalization-shsh]] *(boot-chain module — slug introduced in Part 02)*.
...
- **Command Line Tools ≠ Xcode.** `xcode-select --install` gives `git`/`clang`/`sqlite3` but **no Simulator runtimes, no iOS SDK, and `#Preview` macros fail to resolve** (`PreviewsMacros … not found`). For this course install the *full* Xcode and `sudo xcode-select -s /Applications/Xcode.app`. (See [[ios-xcode-and-the-build-system]].)
...
- **`add-root-cert` trusts a CA inside the Simulator only.** It does nothing to a real device (which needs the profile installed *and* manually enabled under Settings → General → About → Certificate Trust Settings, and still hits app-level pinning). Don't generalize the Simulator TLS lab to device interception without the extra steps in [[traffic-interception-and-tls]].
- **Pairing is not free.** Trusting a device writes host-side pairing records and a device-side trust relationship — an evidentiary act. On real casework, sequence imaging and pairing deliberately ([[ios-forensics-landscape-and-authorization]]).
...
---
*Related lessons: [[macos-to-ios-mental-model-reset]] | [[simulator-internals-and-on-disk-filesystem]] | [[ios-xcode-and-the-build-system]] | [[logical-acquisition-with-libimobiledevice]] | [[the-acquisition-taxonomy]] | [[knowledgec-db-deep-dive]] | [[biome-and-segb-streams]] | [[dynamic-analysis-with-frida]] | [[traffic-interception-and-tls]]*
