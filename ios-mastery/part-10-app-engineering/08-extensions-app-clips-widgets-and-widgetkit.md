...
In [[the-app-bundle-and-ipa-structure]] you saw that `PlugIns/` inside a `.app` holds `.appex` bundles, and in [[the-app-sandbox-from-the-developer-side]] you learned the single most important sentence about them: **an app extension is a separate process in a separate bundle with its own sandbox and its own entitlements — not part of the host app.** This lesson is that sentence, fully unpacked.
...
| **Shared keychain group** | `kSecAttrAccessGroup` on a `keychain-access-groups` / App-Group group | Tokens/creds shared with the extension ([[keychain-on-ios]]). |
...
> 🔬 **Forensics note:** When triaging a third-party keyboard, two questions: (1) does its `Info.plist` declare `RequestsOpenAccess`? and (2) is Full Access actually granted? The grant state and the keyboard's learned-text / autocorrection store are device artifacts covered in [[notifications-keyboard-and-misc-stores]]. A keyboard with Full Access + network is, by capability, able to exfiltrate everything typed in every app — flag it and check its App Group and its server endpoints. Note that secure-text fields (`isSecureTextEntry`, i.e. password fields) fall back to the system keyboard, so a third-party keyboard does *not* see password entry — a useful scoping fact.
...
2. **Remote (the important one):** an **APNs push with `apns-push-type: liveactivity`** carries a new `ContentState`, addressed to the token from `activity.pushToken`. This updates the activity **even when the app isn't running** ([[apple-account-icloud-and-apns]]).
...
The copy-before-query discipline ([[the-app-bundle-and-ipa-structure]] forensic hygiene) applies to App Group databases exactly as to the app's own — a bare `SELECT` opens a write lock and spawns `-wal`/`-shm`.
...
3. Cross-reference the recovered content with the app's own message store ([[communications-imessage-and-sms]]). Note evidence present in the App Group that is **absent** from the app's `Data/Application/<UUID>`.
...
- **Extension and host can install/uninstall independently in the bundle, but share a version.** A re-signed or tampered build can ship an `.appex` whose entitlements *exceed* what the host's profile would justify — diff each extension's signed entitlements against the embedded profile ([[the-code-signature-blob-and-entitlements-on-ios]]); a `com.apple.private.*` key on a "normal" extension is a tamper/sideload indicator just as it is on the app.
...
---
*Related lessons: [[the-app-sandbox-from-the-developer-side]] | [[the-app-bundle-and-ipa-structure]] | [[app-lifecycle-scenes-and-background-execution]] | [[processes-mach-xpc]] | [[app-sandbox-and-filesystem-layout]] | [[communications-imessage-and-sms]] | [[notifications-keyboard-and-misc-stores]] | [[apple-account-icloud-and-apns]] | [[networkextension-and-vpn]] | [[shortcuts-and-the-automation-surface]] | [[third-party-app-methodology]]*
