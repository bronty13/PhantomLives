...
For your device-free workflow, the first three are the whole world: `generic/platform=iOS` to *archive* (compile + sign for device, no device needed to build), and the Simulator forms to actually *run*. The presence or absence of the literal ` Simulator` token is the entire device/Simulator switch — and `xcrun simctl list devices` enumerates the named Simulators you can target. (Simulator internals and the on-disk container layout are [[01-simulator-internals-and-on-disk-filesystem]].)
...
These are *source-level* and *diagnostic-level* changes; they do not change the four-stage pipeline. But they explain the shape of the Swift you will build in [[02-swift-swiftui-uikit-and-app-architecture]] and read when reversing 2026-era apps.
...
---
*Related lessons: [[01-simulator-internals-and-on-disk-filesystem]] | [[04-the-app-bundle-and-ipa-structure]] | [[06-code-signing-and-provisioning-in-depth]] | [[07-frameworks-dylibs-and-dynamic-linking]] | [[00-mach-o-arm64-deep-dive]] | [[03-fairplay-encryption-and-decrypting-app-store-apps]]*
