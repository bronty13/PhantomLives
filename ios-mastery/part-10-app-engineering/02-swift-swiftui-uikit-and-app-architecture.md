...
The split (iOS 13+) is the important part: **the `AppDelegate` owns *process* events; the `SceneDelegate` owns *UI* events** and owns the `UIWindow`. A single process can have multiple scenes (the basis of iPad multi-window — see [[windowing-multitasking-and-external-display]]). Scenes are declared in `Info.plist` under the `UIApplicationSceneManifest` key.
...
For richer Swift type recovery than `nm`, use a Swift-aware dumper (covered in depth in [[static-analysis-class-dump-and-disassemblers]]):
...
---
*Related lessons: [[ios-xcode-and-the-build-system]] | [[app-lifecycle-scenes-and-background-execution]] | [[the-app-bundle-and-ipa-structure]] | [[static-analysis-class-dump-and-disassemblers]] | [[mach-o-arm64-deep-dive]] | [[dynamic-analysis-with-frida]] | [[third-party-app-methodology]]*
