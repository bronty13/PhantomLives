...
> ⚠️ **ADVANCED:** "No FairPlay in the Simulator" does **not** mean you can crack App Store apps here. You *cannot install a real App Store `.ipa` into the Simulator at all* — those are device-arm64 + FairPlay-encrypted and the platform marker is wrong. The Simulator only runs apps **built for the simulator** (your own Xcode builds, or open-source apps you compile yourself). Decrypting a shipped App Store binary is a device-side, FairPlay-memory-dump job covered in [[fairplay-encryption-and-decrypting-app-store-apps]]. The Simulator's value for RE is *structure and schema fidelity*, not DRM bypass.
...
Its `MCMMetadataIdentifier` key holds the bundle id (e.g. `com.example.MyApp`). This is the **same `mobile_container_manager` mechanism** that names containers on a real iPhone, so the technique you learn here transfers verbatim to a full-filesystem device acquisition — see [[app-sandbox-and-filesystem-layout]].
...
|---|---|
| Secure Enclave / SEP | No real keybag, no class keys, no biometric crypto — see [[sep-sepos-deep-dive]] |
...
3. `otool -L` it and pick two linked frameworks; confirm with `find` that those live inside the runtime's `RuntimeRoot`. Contrast, in writing, with how a *device* binary would resolve via the on-device dyld shared cache ([[the-dyld-shared-cache]]).

### Lab 5 — Author a forensic query in the lab, then port it (Simulator → read-only walkthrough)

1. In the simulator Safari, visit three sites. Then `find … -name History.db`, `cp` it, and write a `SELECT` joining `history_visits`↔`history_items` with the `+ 978307200` epoch conversion ([[the-ios-timestamp-zoo]]).
...
- **You cannot install a real App Store `.ipa`.** Wrong platform slice + FairPlay. Simulator apps are simulator builds only. App-decryption is a device job ([[fairplay-encryption-and-decrypting-app-store-apps]]).
...
---
*Related lessons: [[app-sandbox-and-filesystem-layout]] | [[ios-xcode-and-the-build-system]] | [[the-app-bundle-and-ipa-structure]] | [[mach-o-arm64-deep-dive]] | [[the-dyld-shared-cache]] | [[traffic-interception-and-tls]] | [[the-ios-timestamp-zoo]]*
