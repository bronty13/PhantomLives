...
The honest answer has two halves, and both matter. As a **builder**, you should know that Apple has shipped a real, sanctioned path to build *and submit* an App Store app entirely on-device — that's a non-trivial capability with its own provenance trail. As a **forensicator**, you should know exactly *why* the iPad can never run your acquisition/analysis stack (it's the same AMFI/JIT mechanism you learned in [[04-code-signing-amfi-entitlements]], not a missing feature Apple will someday add), so you never waste field time trying — and so you recognize the artifacts an iPad-as-creation-tool leaves behind when one turns up in an investigation as the device that *authored* something.
...
| Development cert + provisioning profile | Keychain / provisioning store ([[04-code-signing-amfi-entitlements]]) | The Apple Account/Team that signed builds on this device |
...
- **Remote desktop into the Mac.** Screen Sharing / VNC / Jump Desktop / Screens, plus Apple's own **Sidecar** and **Universal Control** ([[04-continuity-with-the-mac]]), let you drive the Mac's full Xcode and forensics GUI from the iPad. You are looking at the Mac; the iPad renders pixels.
...
2. Contrast: explain (from [[04-code-signing-amfi-entitlements]] / [[07-dyld-shared-cache-and-amfi]]) why WebKit's `JavaScriptCore` is the canonical holder of the JIT grant and why Safari can therefore JIT JavaScript while a sideloaded emulator cannot.
...
- **Shortcuts is not a sandbox escape.** It can only call App Intents an app chose to publish and only touch files through granted scopes. Don't model it as a shell; do model the *exposed-intent surface* as the actual attack surface ([[05-the-sandbox-and-tcc]]).
...
- Apple — iPadOS 26 newsroom + WWDC25 sessions on the windowing model, the menu bar, and Background Tasks (the windowing mechanics are in [[01-windowing-multitasking-and-external-display]])
...
---
*Related lessons: [[00-ios-xcode-and-the-build-system]] | [[00-how-ipados-diverges-from-ios]] | [[04-code-signing-amfi-entitlements]] | [[00-shortcuts-and-the-automation-surface]] | [[01-windowing-multitasking-and-external-display]] | [[05-dynamic-analysis-with-frida]] | [[04-the-app-bundle-and-ipa-structure]]*
