...
| `bluetoothd` | `com.apple.bluetoothd` | Emits and scans the BLE Continuity advertisements (`com.apple.bluetooth.wirelessproximity`); owns the pairing/bond stores from [[04-wifi-bluetooth-and-proximity]]. |
...
| Phone / SMS relay | BLE + IDS | local Wi-Fi + APNs | Phone/Messages stores ([[04-communications-imessage-and-sms]], [[05-call-history-voicemail-contacts-interactions]]) |
...
> 🔬 **Forensics note (the join key):** Pull the DSID from `Accounts3.sqlite` on the iPad, then look for the *same* DSID on any Mac/iPhone image you also hold. A DSID match is co-ownership; it also tells you which **iCloud account to target** for an Advanced-Data-Protection-aware cloud-acquisition request ([[06-icloud-acquisition-and-advanced-data-protection]], [[07-apple-account-icloud-and-apns]]) and which account Apple legal process would key on. The Apple-Account email/phone in this store is contact-identifiable PII — handle accordingly.
...
- **`/private/var/root/Library/Lockdown/pair_records/<UDID>.plist`** — every Mac/PC this device has trusted over USB (the escrow-bag pairing that also gates `libimobiledevice` acquisition; [[04-logical-acquisition-with-libimobiledevice]]). A USB trust record is itself a device-association fact: this computer paired with this iPad.
...
3. For a `0x0C` frame, find the clipboard flag and the sequence number; correlate its stepping with a device unlock. **Fidelity caveat:** payloads are AES-GCM-encrypted under a per-account key — you read *metadata and cadence*, not plaintext. (Full beacon dissection lives in [[04-wifi-bluetooth-and-proximity]].)
...
- **Epoch discipline (the 31-year tell).** These stores mix epochs: plist `<date>` values are Mac Absolute Time that `plutil` renders; `purplebuddy`/`data_ark` setup times are often **Unix milliseconds**; SQLite `LastSeenTime` columns may be Mac Absolute seconds (add `978307200`). Verify per field before trusting a converted value (see [[00-the-ios-timestamp-zoo]]).
...
- **All of it is Data-Protection-gated.** The Accounts DB, IDS state, migration plists, and pairing DBs are user-class data — **unreadable from a BFU image** until the passcode is recovered. Confirm lock-state before promising an association timeline ([[02-bfu-vs-afu-and-data-protection-classes]]).
...
---
*Related lessons: [[04-wifi-bluetooth-and-proximity]] | [[00-how-ipados-diverges-from-ios]] | [[01-windowing-multitasking-and-external-display]] | [[07-apple-account-icloud-and-apns]] | [[05-backup-restore-migration-and-transfer]] | [[01-building-a-unified-timeline]] | [[02-bfu-vs-afu-and-data-protection-classes]]*
