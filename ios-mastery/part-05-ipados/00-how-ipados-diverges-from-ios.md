...
|---|---|---|
| XNU kernel, BSD layer, Mach | Identical | [[00-xnu-on-mobile]] |
| Secure boot chain, Image4, SHSH | Identical | [[01-boot-chain-securerom-iboot]] |
| APFS volume roles, sealed system volume | Identical | [[03-apfs-on-ios-volumes]] |
| Sandbox, AMFI, code-signing, entitlements | Identical | [[04-code-signing-amfi-entitlements]] |
| Secure Enclave, Data Protection, keybags | Identical | [[02-data-protection-and-keybags]], [[01-sep-sepos-deep-dive]] |
| Per-app container layout under `/private/var/mobile/Containers/` | Identical | [[08-filesystem-layout-and-containers]], [[00-app-sandbox-and-filesystem-layout]] |
...
| Backup format (`Manifest.db`, domain/relative-path hashing) | Identical | [[03-the-itunes-finder-backup-format]] |
| BFU/AFU lock states, 72-hour inactivity reboot | Identical | [[03-passcode-bfu-afu-and-inactivity]] |
...
- **External-volume access leaves traces even when the files are gone.** The system logs mount/attach events (queryable in the unified log, see [[12-unified-logs-sysdiagnose-crash-network]]), and File Provider metadata can retain item records for content that lived on a now-removed drive.
...
The memory split has a lifecycle consequence. On iPhone, the background-execution model you learned in [[06-memory-jetsam-app-lifecycle]] is deliberately starved: short `beginBackgroundTask` windows, opportunistic `BGTaskScheduler` jobs, and a quick march to *suspended* — because the device needs to reclaim memory by termination. iOS/iPadOS 26 introduces **`BGContinuedProcessingTaskRequest`**: a user-initiated, foreground-started task (export a video, upload a large file) that **continues after you switch away and is presented in a system-drawn, Live-Activity-style progress UI** — the Dynamic Island / a notification banner on iPhone, a progress indicator on iPad. The system, not your app, draws that UI; the task conforms to `ProgressReporting` and **must keep reporting measurable progress or the system expires it** (a stalled task is killed). On supported devices it can also be granted **background GPU access** (opt-in via the background-GPU capability + a runtime check of the scheduler's supported-resources). This is the closest iPadOS has come to a Mac-style "keep working while I do something else" background job, and it leans directly on the M-series memory headroom — an app that's paging to swap can keep grinding where an iPhone would have suspended or jetsammed it.
...
- The **iPad Air** and **iPad Pro** run **M-series** SoCs (M1 → M5), the *same desktop-class designs as the Mac* (see [[00-soc-lineup-and-device-matrix]]). The 2025 **iPad Pro (M5)** ships with **12 GB** of LPDDR5X on the 256/512 GB models and **16 GB** on the 1 TB/2 TB models.
...
The capability that matters most is **Virtual Memory Swap**, introduced with iPadOS 16 on M-series iPads. In [[06-memory-jetsam-app-lifecycle]] you learned the iPhone memory model: **there is no swap.** iOS relies on the in-kernel compressor (`vm_compressor`, WKdm-style compression of dirty pages) and, when that isn't enough, **Jetsam** kills the lowest-priority app outright. Memory pressure on iOS is resolved by *termination*, not paging.
...
The transport underneath is the same proximity stack as the rest of the Apple ecosystem — historically **AWDL**, now migrating to **Wi-Fi Aware** (see [[04-wifi-bluetooth-and-proximity]]) — plus iCloud-mediated state. The point for this lesson: an iPad is rarely a standalone island; it is usually one node of a Mac+iPad+iPhone mesh, and evidence flows between them.
...
```
The `iPadN,N` board ID resolves to the SoC (and therefore to "does this device have swap?") via the device matrix in [[00-soc-lineup-and-device-matrix]].
...
2. Map the `iPadN,N` board ID to its SoC using the matrix in [[00-soc-lineup-and-device-matrix]].
...
- The lineup is **split by silicon**: A-series base iPad/mini behave like phones; M-series Air/Pro behave like Macs — most importantly, they have **Virtual Memory Swap**, which changes the [[06-memory-jetsam-app-lifecycle]] picture from "compress-then-kill" to "compress-then-page-then-kill."
...
---
*Related lessons: [[06-memory-jetsam-app-lifecycle]] | [[02-macos-to-ios-mental-model-reset]] | [[00-soc-lineup-and-device-matrix]] | [[02-files-external-storage-and-document-providers]] | [[01-windowing-multitasking-and-external-display]] | [[04-continuity-with-the-mac]] | [[05-full-file-system-acquisition]] | [[10-eu-dma-sideloading-and-alternative-marketplaces]] | [[09-mail-notes-calendar-reminders]] | [[00-the-ios-timestamp-zoo]]*
