...
> 🔬 **Forensics note:** On-demand match criteria leak environment intelligence. `SSIDMatch` lists the **names of Wi-Fi networks** the org expected the device on (often "do *not* tunnel on these trusted SSIDs"); a `URLStringProbe` reveals an **internal hostname** (e.g. `https://probe.corp.acme.internal`) that confirms private infrastructure; `DNSDomainMatch` reveals internal domains. These corroborate the known-networks evidence you pull from [[04-wifi-bluetooth-and-proximity]] and the cellular picture from [[06-cellular-baseband-esim-and-identifiers]].
...
1. From a full-file-system extraction (or decrypted IPAs — see [[03-fairplay-encryption-and-decrypting-app-store-apps]]), iterate the installed third-party app bundles.
...
- **AWDL / Wi-Fi Aware, AirDrop, and Continuity are *not* NetworkExtension.** Peer-to-peer link-layer traffic and the proximity stack don't pass through NECP VPN/relay policy and won't appear in these stores — chase them in [[04-wifi-bluetooth-and-proximity]] and [[05-find-my-and-the-ble-mesh]], not the VPN config.
...
- **Per-app VPN implies management.** If you see `com.apple.vpn.managed.applayer`, treat the device as supervised/MDM-enrolled and go pull the rest of the management posture ([[02-mdm-supervision-and-abm]], [[04-configuration-profiles-and-mobileconfig]]).
...
- **Lockdown Mode and supervised restrictions can block NE.** A device in **Lockdown Mode** or under restriction profiles may refuse to install certain NE configs; absence of an expected config can be a *policy* artifact, not a user choice ([[06-lockdown-mode-and-enterprise-posture]], [[09-advanced-protections-lockdown-sdp-adp]]).
...
---
*Related lessons: [[00-the-ios-networking-stack]] | [[04-configuration-profiles-and-mobileconfig]] | [[02-traffic-interception-and-tls]] | [[02-mdm-supervision-and-abm]] | [[12-unified-logs-sysdiagnose-crash-network]] | [[05-full-file-system-acquisition]]*
