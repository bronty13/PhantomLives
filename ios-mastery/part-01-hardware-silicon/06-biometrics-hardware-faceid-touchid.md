...
> 🔬 **Forensics note:** None of the TrueDepth raw data is recoverable from a disk image — there is no `FaceID.depthmap` file, by design (see "what the hardware refuses to do" below). What you *can* recover on a real device is **operational telemetry**: unified-log entries from `biometrickitd` and the SEP's biometric subsystem record match/no-match **events and timestamps** (not the biometric content). In a timeline ([[01-building-a-unified-timeline]]) those events corroborate *attended* unlocks — a successful Face ID match at 02:14 is strong evidence a face was in front of the device at 02:14 — without ever exposing the face itself. Capture them with a sysdiagnose ([[12-unified-logs-sysdiagnose-crash-network]]); they are absent from a Simulator (the Simulator has no `biometrickitd` match pipeline — see Labs).
...
`biometryType` returning `.touchID`/`.faceID`/`.opticID`/`.none` is the *same* `LAContext` API an iOS app calls — you are exercising the real framework against a real SEP, just on Mac hardware. The values are the actual enum raw values you'll parse in iOS app code later ([[00-the-ios-security-model]]).
...
# (see [[04-logical-acquisition-with-libimobiledevice]]).
```
...
3. Connect it to acquisition: if biometrics are unavailable, what authentication remains, and why does that make the device's data-protection state ([[02-bfu-vs-afu-and-data-protection-classes]]) *harder*, not easier, to defeat?
...
1. Mount a public iOS reference image (Josh Hickman / Digital Corpora) or an extracted sysdiagnose. Search the unified logs ([[12-unified-logs-sysdiagnose-crash-network]]) for `biometrickitd` and the SEP biometric subsystem.
...
- **Conflating "biometrics disabled" with "user did it."** A disabled Face ID/Touch ID can mean: a deliberate demote-to-passcode, the 48 h/5-tries/Emergency-SOS rules ([[03-passcode-bfu-afu-and-inactivity]]) — **or a hardware sensor/display swap that broke the pairing.** Check Parts & Service History before attributing intent.
...
- Cross-references: [[02-secure-enclave-hardware]] (the SEP this all rides on), [[07-biometrics-security-architecture]] (enrollment + match→key-release, Part 03), [[03-passcode-bfu-afu-and-inactivity]] (when biometrics force-disable), [[06-kernel-hardening-pac-sptm-txm-mie]] (the A19 hardening stack the 2026 SNE rides on).

---
*Related lessons: [[02-secure-enclave-hardware]] | [[07-biometrics-security-architecture]] | [[03-passcode-bfu-afu-and-inactivity]] | [[02-bfu-vs-afu-and-data-protection-classes]] | [[00-the-ios-security-model]] | [[00-ios-forensics-landscape-and-authorization]]*
