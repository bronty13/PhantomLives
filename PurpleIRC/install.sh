#!/bin/bash
# Reinstall the freshly-built PurpleIRC.app into /Applications/.
#
# Usage:
...
# A graceful `osascript ... quit` can be blocked indefinitely by a
# quit-confirmation dialog (PurpleIRC has one), an unsaved-changes prompt, or
# a hung run loop. When that happens the old process keeps running and a plain
# `open` just re-focuses the STALE copy — the new binary on disk never runs.
# So we force-kill, *wait until the process is actually gone*, launch a NEW
# instance with `open -n`, and then PROVE the running process started after the
# new binary was written. Any failure is fatal and loud.
set -euo pipefail
...
# Match the running process by its executable path so we never kill an
# unrelated process that merely has "PurpleIRC" in its argv.
...
echo "Terminating any running $APP (force, with confirmation)..."
...
echo "Launching $DST"
open -n "$DST"                     # -n: new instance, never re-focus

# --- 4. Prove the running process is the new one ---------------------------
# Wait for it to come up, then assert its start time is >= the binary mtime.
RUNNING_PID=""
for _ in $(seq 1 50); do           # up to ~10s for launch
