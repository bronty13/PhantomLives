        ...
        List(selection: Binding(
                ...
                Section("Channels") {
                    ...
                    .onMove { source, destination in
                        model.activeConnection?.moveBuffers(
                            kind: .channel, from: source, to: destination)
                    }
                }
            }
            ...
            if !saved.isEmpty {
                Section("Saved") {
                    ...
                    .onMove { source, destination in
                        model.settings.moveSavedChannels(
                            from: source, to: destination,
                            selectedServerID: model.settings.settings.selectedServerID)
                    }
                }
            }
            ...
            if !addresses.isEmpty {
                Section("Contacts") {
                    ...
                    .onMove { source, destination in
                        model.settings.moveAddressBook(from: source, to: destination)
                    }
                }
            }
        }
