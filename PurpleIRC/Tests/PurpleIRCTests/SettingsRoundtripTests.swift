...
struct SettingsRoundtripTests {
    ...
    // MARK: - sidebarSectionOrder

    @Test func sidebarSectionOrderDefaultMatchesEnumOrder() {
        let s = AppSettings()
        #expect(s.sidebarSectionOrder == SidebarSection.defaultOrder)
    }

    @Test func sidebarSectionOrderCustomOrderRoundtrips() {
        var s = AppSettings()
        s.sidebarSectionOrder = [.contacts, .saved, .privateBuffers, .channels, .networks]
        let r = roundtrip(s)
        #expect(r?.sidebarSectionOrder == [.contacts, .saved, .privateBuffers, .channels, .networks])
    }

    @Test func sidebarSectionOrderMissingFieldFallsBackToDefault() throws {
        // Old payloads predate the field; loading must produce the
        // factory order, not an empty array.
        let json = """
        {"servers":[],"themeID":"classic"}
        """
        let s = try JSONDecoder().decode(AppSettings.self, from: json.data(using: .utf8)!)
    ...
    @Test func sidebarSectionOrderUnknownRawValuesAreDropped() throws {
        // Forward-compat: a future build saves a raw value this build
        // doesn't know about. The unknown entry must be silently
        // discarded and the missing legitimate entries appended in
        // default order — never truncate the user's working list.
        let json = """
        {"sidebarSectionOrder":["contacts","does-not-exist","networks"]}
        """
        let s = try JSONDecoder().decode(AppSettings.self, from: json.data(using: .utf8)!)
        // contacts and networks first (in saved order), missing entries appended.
        #expect(s.sidebarSectionOrder.first == .contacts)
        #expect(s.sidebarSectionOrder.contains(.networks))
        #expect(s.sidebarSectionOrder.contains(.channels))
        #expect(s.sidebarSectionOrder.contains(.privateBuffers))
        #expect(s.sidebarSectionOrder.contains(.saved))
        #expect(s.sidebarSectionOrder.count == SidebarSection.allCases.count)
    }

    @Test func sidebarSectionOrderDuplicateEntriesAreDeduplicated() throws {
        // Pathological payload: the same section twice. Normalize must
        // collapse to one and add any missing.
        let json = """
        {"sidebarSectionOrder":["channels","channels","contacts"]}
        """
        let s = try JSONDecoder().decode(AppSettings.self, from: json.data(using: .utf8)!)
        #expect(s.sidebarSectionOrder.count == SidebarSection.allCases.count)
        #expect(s.sidebarSectionOrder.filter { $0 == .channels }.count == 1)
    }

    @Test func sidebarSectionOrderNormalizeStableForFullValidInput() {
        let custom: [SidebarSection] = [.saved, .channels, .networks, .contacts, .privateBuffers]
        let normalized = SidebarSection.normalize(custom)
        #expect(normalized == custom)
    }

    @Test func sidebarSectionOrderNormalizeAppendsMissing() {
        let partial: [SidebarSection] = [.contacts, .saved]
        let normalized = SidebarSection.normalize(partial)
        // First two stay where they were; remaining fill in defaultOrder order.
        #expect(normalized[0] == .contacts)
        #expect(normalized[1] == .saved)
        #expect(Set(normalized) == Set(SidebarSection.allCases))
        #expect(normalized.count == SidebarSection.allCases.count)
    }
}
