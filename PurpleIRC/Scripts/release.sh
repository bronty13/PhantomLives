...
# extractor also yields a Gatekeeper-valid bundle. Extract with `unzip` and
# assert no ._* detritus in Frameworks/, a valid staple, and strict codesign.
...
codesign --verify --deep --strict "$GATE_APP" 2>/tmp/codesign-gate.err \
    || die "release zip fails strict codesign after unzip (unsealed-framework failure):
$(sed 's/^/         /' /tmp/codesign-gate.err)"
rm -rf "$GATE_DIR"
