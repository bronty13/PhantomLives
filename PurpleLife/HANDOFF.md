...
**Library, not built-ins.** The 251 curated schemas live in `SchemaLibrary.entries` as a separate catalog from `SchemaSeed.allTypes`. Built-ins ship installed on first launch and own stable ids (`"Person"`, `"Book"`, etc.) the app can recognize; library entries import as ordinary user-defined types with fresh UUIDs. Mixing the two — e.g. shipping the library as additional built-ins, hidden by default — would have produced a 250+-type sidebar by default, leaked the library's ids into the CloudKit schema-sync stream, and stolen the user's freedom to delete a library entry they imported and changed their mind about. Keeping them separate means the library is purely an *invitation* (via the gallery sheet) — the registry stays exactly as the user shaped it.

**Catalog split across two files.** The original first-batch (52 entries) sits inline in `Models/SchemaLibrary.swift` as `coreEntries`; the remaining 199 entries live in `Models/SchemaLibrary+ExtendedCatalog.swift` as `extendedEntries`. `SchemaLibrary.entries` is a computed property that concatenates them. Reason: a single Swift file with 250+ static-let literals risks pushing Swift's type-checker over its complexity cliff (and slows incremental builds noticeably even when it doesn't). Splitting at the file boundary kept each file under ~1700 lines and let me add the second batch without recompiling the first. Helpers that used to be `fileprivate` (`selectField`, `makeType`) became `internal` so the extension file can reach them — that's the only API-surface change.

**Fresh ids on every import path.** Three things have to be true and aren't separable: (1) the same library entry can be imported twice without collision; (2) a user can share a `.purplelifeschema.json` with another Mac via AirDrop and it imports cleanly even though both peers may have a copy of the same source-of-truth template; (3) round-tripping (export → edit → import) never overwrites the original. The fix is uniform: every entry point through `Entry.materialize()` / `SchemaIO.freshenIds` / `SchemaRegistry.importTypes` regenerates the type id and every `FieldDef.id`. The `key` on each field stays stable (derived from name, snake-cased) so existing record data — which keys field values by `key`, not by id — survives the new shape unchanged.

**View-default keys validated by test.** `kanbanGroupKey` only makes sense on a `.select` field; `calendarDateKey` only on `.date`/`.dateTime`; `galleryAttachmentKey` only on `.attachment`. The catalog is 251 entries deep and easy to typo. `SchemaLibraryTests.testKanbanGroupKeyAlwaysPointsAtSelectField` (and siblings) walks every entry on every test run; across the two rounds of catalog expansion the test caught five real bugs (podcast / place-visited / fish-caught / garden-harvest with kanban keys on text fields, appliance with a calendar key on a number field) and every one was fixed before the catalog went out.

**Envelope, not bare array.** `SchemaIO.Envelope` wraps the type list in `{ format, exportedAt, types }`. The `format` field gives forward-compat headroom — a future v2 can add per-type extension fields, palette colors, layout hints — without breaking old importers that strictly check for `purplelife.schema.v1`. The decoder also accepts a bare `[ObjectType]` array as a kindness to hand-rolled files, but the canonical write path always wraps.

**Reset preserves data.** The new `SchemaRegistry.resetBuiltInsToDefaults()` rewrites built-in `ObjectType`s back to the `SchemaSeed` shape and fans the rewritten types out to CloudKit via `pushType`. User-defined types are untouched; per-device `hiddenBuiltInIds` survives. Record data inside `ObjectRecord.fieldsJSON` keys by `FieldDef.key`, which is name-derived and stable — so a "Title" field renamed back to "Title" still reads the user's stored titles after the reset. If the user had renamed "Title" to "Subject" themselves, the reset *will* surface their old "title"-keyed data again under the seed's "Title" field; that's the right thing (data preservation > rename retention) but worth flagging.

**Schema-sync echo on reset.** `resetBuiltInsToDefaults` deliberately calls `sync?.pushType` for each rewritten built-in. Without it, a multi-Mac user resets on Mac A, switches to Mac B with the customized version still synced, and the next CloudKit pull restores Mac B's customizations on Mac A via LWW (because Mac B's per-type `updatedAt` is newer than the freshly-reset Mac A's). The reset pushing each type with `updatedAt = now` flips the LWW comparison in favor of the reset, so peers reconcile to the seed shape. This is the same pattern as `restore(snapshot:fanOut:)` in the undo path.
