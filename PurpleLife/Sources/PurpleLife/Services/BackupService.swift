...
/// Auto-backup service for PurpleLife.
///
/// PhantomLives convention: every app that owns persistent user data runs an
/// automatic backup on launch. Default location: `~/Downloads/PurpleLife backup/`.
...
/// directory and retention in Settings → Backup (Phase 1 ships the service +
/// tests; the Settings pane lands later in Phase 1).
@MainActor
enum BackupService {
    ...
    /// name format is `PurpleLife-yyyy-MM-dd-HHmmss.zip`. Used by both the
    /// retention trim and the listing UI to recognize "our" archives.
    static let archivePrefix = "PurpleLife-"
    ...
    /// debounce window. Errors are swallowed (logged via NSLog) — the app
    /// must never refuse to launch because backup failed. Public entry point
    /// used from `AppState.init`.
    static func runOnLaunchIfDue(settingsStore: SettingsStore) {
        ...
        do {
            ...
            NSLog("PurpleLife: backup-on-launch wrote \(url.lastPathComponent)")
        } catch {
            NSLog("PurpleLife: backup-on-launch failed — \(error.localizedDescription)")
        }
    }
    ...
    static func runBackup(supportDir: URL, backupDir: URL) throws -> URL {
        ...
        let tempZip = fm.temporaryDirectory
            .appendingPathComponent("purplelife-backup-\(UUID().uuidString).zip")
    ...
    enum RestoreError: Error, LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .missingDatabase:        return "Backup didn't contain purplelife.sqlite"
    ...
    nonisolated static func verifyArchive(at archiveURL: URL) throws -> VerifyResult {
        ...
        let dbCandidate = staging.appendingPathComponent("purplelife.sqlite")
        ...
        do {
            ...
            try pool.read { db in
                ...
                objectCount = try Int.fetchOne(db, sql: "SELECT COUNT(*) FROM objects") ?? 0
            }
        } catch {
    ...
    static func restoreArchive(at archiveURL: URL, into supportDir: URL) throws {
        ...
        let staging = fm.temporaryDirectory
            .appendingPathComponent("pl-restore-\(UUID().uuidString)", isDirectory: true)
        try fm.createDirectory(at: staging, withIntermediateDirectories: true)
        defer { try? fm.removeItem(at: staging) }
