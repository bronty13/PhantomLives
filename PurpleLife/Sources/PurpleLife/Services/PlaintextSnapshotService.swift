...
/// Lets us round-trip the heterogeneous `fields_json` dictionary
/// (strings, numbers, bools, arrays, nested dicts, null) through
/// JSONEncoder without a per-key type-erasure dance. Encoder writes
/// the underlying JSON shape; decoder is provided for completeness so
/// tests can deserialize the round-tripped envelope.
struct AnyCodable: Codable {
    ...
    private static func encodeAny(_ value: Any, into container: inout SingleValueEncodingContainer) throws {
        if value is NSNull {
            ...
            // JSONSerialization hands back NSNumber for many numeric
            // values. Detect Bool by CoreFoundation type — NSNumber wraps
            // Bool with CFBooleanGetTypeID().
