#!/bin/bash
# Reinstall the freshly-built PurplePeek.app into /Applications/ and prove the running
# instance is the new build (PhantomLives install.sh standard: force-kill → replace →
# relaunch → prove freshness via process-start-time >= binary mtime).
#
...
SRC="$PWD/PurplePeek.app"
DST="/Applications/PurplePeek.app"
OPEN_AFTER=1
for arg in "$@"; do
    case "$arg" in
...
# ---- Force-kill any running instance (a graceful quit can be blocked indefinitely) ----
