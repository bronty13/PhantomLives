...
/// (or pane) width that's smaller than its declared minimum — or larger than the current
/// window — and stranding the user with a broken layout on every relaunch.
///
/// This helper is the canonical fix for every PhantomLives macOS app. PurplePeek's
/// top-level chrome is a manual `HStack` (not `NavigationSplitView`), but the guard still
/// runs to keep any nested split state and stale `.savedState` clean across launches.
///
/// On every launch (call from `applicationWillFinishLaunching`, BEFORE the first window):
///  1. Preflight purge: strips all `"NSSplitView Subview Frames *"` UserDefaults keys and
///     wipes the bundle's `.savedState` dir when any stale frame key was found.
///  2. Versioned one-shot reset: when `resetVersion` increments, wipes the entire
///     window-state surface so a layout change invalidates every install on next launch.
