...
| `Ircle/` (Swift, SwiftUI macOS app — a nostalgic recreation of the classic Mac *Ircle* IRC client) | `./build-app.sh` → `Ircle.app` in `/Applications/` (plain SwiftPM, depends on `../IRCKit`, Developer-ID-or-adhoc-signed, build + install + relaunch + freshness proof; code-generated icon; `--no-install` / `--no-open` / `BUILD_ONLY=1` opt-outs). Clean-room Platinum/Graphite single-window UI (Channelbar, message area, nick list, input line) over an `IrcleSession` session layer on IRCKit. TLS/SASL/proxy; channels/queries/auto-join; slash commands; auto-backup standard (settings → `~/Downloads/Ircle backup/`). mIRC colors stripped to plain text in MVP. Not a literal port (Ircle 3.x is closed-source) — see `Ircle/README.md`. | `./run-tests.sh` — swift-testing, 18 tests (backup debounce/retention/auto-create/list-order + verify, nick-list maintenance, user ordering, case-folding, session plumbing). |
