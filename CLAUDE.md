...
| `PurpleMirror/` (Swift, SwiftUI **menu-bar** `LSUIElement` app) | `./build-app.sh` → `PurpleMirror.app` in `/Applications/` (plain SwiftPM, Developer-ID-or-adhoc-signed, build + install + relaunch; code-generated icon). A thin GUI over `sync-md-to-obsidian.sh` + its launchd agent `com.phantomlives.obsidian-sync`: menu-bar status glyph, Sync Now, interval/enable settings, a live-tail log viewer, macOS failure notifications, and **Sparkle 2 auto-update** (`Scripts/release.sh` → notarized + EdDSA-signed zip + `appcast.xml`; reuses the shared Developer-ID cert / `PurpleDedup-Notary` profile / Sparkle key — see `PurpleMirror/RELEASING.md`). Not sandboxed (manages launchd, reads `~/Library`). Auto-backup standard: exempt (owns no user data beyond two recreatable prefs). See `PurpleMirror/README.md` and `docs/obsidian-setup.md`. | `./run-tests.sh` — swift-testing, 9 tests (log-line / launchctl-print parsing, interval & relative-age formatting, health classification). |
| `PurplePeek/` (Swift, SwiftUI macOS app — media-triage: scan a folder, decide, import to Photos) | `./build-app.sh` → `PurplePeek.app` in `/Applications/` (plain SwiftPM + GRDB, Developer-ID-or-adhoc-signed, build + install + relaunch; code-generated icon; `--no-install` / `--no-open` / `BUILD_ONLY=1` opt-outs). Recursively discovers photos/videos/audio under a folder, per-item decisions (keep/skip, favorite, title, caption, keywords, albums) persisted in SQLite by file path; Folder + Preview modes; imports keepers to Photos via PhotoKit with title/caption/keywords embedded by **exiftool** into a staged copy (PhotoKit can't write them) + favorite/album via PhotoKit; audio is keep-exported to `~/Downloads/PurplePeek/Kept Audio/` (Photos can't hold audio); delete imported/skipped to Trash or permanently; 10 themes; auto-backup standard. Not sandboxed. **"Mirror Photos" rule: option set = what Photos can hold (no star rating).** Needs `exiftool` (optional, `brew install exiftool`) for import metadata. | `./run-tests.sh` — XCTest, 21 tests (migrations + decision-preserving re-scan upsert + cascade delete, discovery classification, backup retention, audio/delete/staging services); sets `DEVELOPER_DIR` to full Xcode so XCTest resolves under Command Line Tools. |
...
| `PurpleChef/` (TypeScript, Electron 31 + Canvas 2D, cross-platform macOS/Windows **game** — Overcooked-style cooking race vs. an AI chef) | `./build-app.sh` → `Purple Chef.app` in `/Applications/` (host-arch, adhoc-signed, build + install + relaunch; `--no-install` / `--no-open` / `BUILD_ONLY=1` opt-outs). `npm install` runs automatically on first invocation. Releases: `scripts/release.sh` (notarized universal2 DMG locally; `purplechef-v*` tag → CI builds the Windows NSIS installer + publishes; see `PurpleChef/RELEASING.md`). Pure-logic game brain lives in `src/shared/` (sim, AI, levels, recipes, prizes — no Electron/DOM imports). All art/SFX generated from code (canvas + WebAudio); icon via `python3 build/make_icon.py`. Auto-runs the launch-time backup standard. | `npm test` — vitest, 49 tests (recipes/levels/orders/path/sim units, headless full-match AI integration per kitchen × difficulty, prizes, backup debounce/retention/auto-create/ordering). `npm run typecheck` for `tsc --noEmit` against both tsconfigs. |
| `IRCKit/` (Swift, SwiftPM **library** — the shared IRC wire engine) | `swift build`. No `.app` (it's a library). Pure Foundation + Network: the RFC 1459 / IRCv3 line parser (`IRCMessage`) + sanitizer/masker (`IRCSanitize`), the `NWConnection` transport (`IRCClient` + `IRCConnectionConfig`/`State`), CAP/SASL state machine (`SASLNegotiator`, internal), SOCKS5/HTTP-CONNECT framer (`ProxyFramer`, internal), the `IRCConnectionEvent` fan-out enum, `SASLMechanism`/`ProxyType`, `IRCText.stripFormatting`, the `DCC` security/parse engine (`parseOffer`, `sanitizeFilename` path-traversal guard, `validatedPeerHost` SSRF guard, offer encoders) + the `DCCDownload`/`DCCUpload`/`DCCChat` transports, and `IRCMask` (hostmask glob matching for ignore lists). Consumed by **both Ircle and PurpleIRC** via `.package(path: "../IRCKit")`. Extracted from PurpleIRC's engine spine; engine types are `public`. (PurpleIRC migrated onto IRCKit in IRCKit Phase 2 — its old engine-file copies are deleted; `IRCConnection` is the only session layer left there. PurpleIRC keeps its own DCC/ignore copies for now; converge later.) | `./run-tests.sh` — swift-testing, 92 tests (parser, sanitizer/masker, SASL, formatting stripper, DCC security + offer parsing, hostmask matching). |
| `Ircle/` (Swift, SwiftUI macOS app — a nostalgic recreation of the classic Mac *Ircle* IRC client) | `./build-app.sh` → `Ircle.app` in `/Applications/` (plain SwiftPM, depends on `../IRCKit`, Developer-ID-or-adhoc-signed, build + install + relaunch + freshness proof; code-generated icon; `--no-install` / `--no-open` / `BUILD_ONLY=1` opt-outs). Clean-room Platinum/Graphite single-window UI (Channelbar, message area, nick list, input line) + a separate Faces window (per-user avatars: assigned image or generated monogram) over an `IrcleSession` session layer on IRCKit. multi-server (one `IrcleSession` per network; Channelbar grouped by server; built-in common-network presets); TLS/SASL/proxy; channels/queries/auto-join; mIRC color/formatting rendering (+ clickable links, custom text/bg colours); a **Clean/Classic interface-style toggle** (Classic adds the nick-list action grid, the `t n i p s m l k r` channel-mode toggle row, Users/Notify tabs, and the input formatting toolbar); **full DCC** (accept + initiate, files + chat — `IrcleDCC` over IRCKit's transports; DCC Transfers window + per-chat windows); **ignore** (`/ignore`, hostmask wildcards via `IRCMask`); **Notify/friends** (ISON-polled); **CTCP + per-event sounds** (`SoundService`); **chat logging + a log viewer** (⌘⇧L); **macOS notifications** (`NotificationService`); **command aliases** (`AliasExpander`); `/away`; WHO-derived nick **hostnames + IRCop markers**; an **in-app manual** (`ManualView` + a small `MarkdownParser`, ships `Resources/Manual.md`, ⌘?); passwords in the **Keychain** (device-only); AppleScript (connect/join/say/current-nickname); Sparkle 2 auto-update (`Scripts/release.sh`; release zip is xattr-stripped + unzip-gated so it's Gatekeeper-clean on any extractor); auto-backup standard. SwiftUI macOS-14 window sizing is unreliable — `AppDelegate` grows degenerate windows on `didBecomeKey`. Slash commands routed in `IrcleModel.submitInput` (global cmds + alias expansion) → `IrcleSession.runCommand` → passthrough. Not a literal port (Ircle 3.x closed-source) — see `Ircle/README.md` + the in-app manual. | `./run-tests.sh` — swift-testing, 135 tests (backup, buffers/nick-list/case-folding, dispatch, mIRC renderer + links, faces, multi-server, presets, credentials, notify/ISON, away, notifications, ignore, DCC offer/transfer/self-guard, CTCP + per-event sounds, aliases, custom colours, WHO, markdown parser). |
| `rsicci/` (TypeScript, Vite + React + TS — single self-contained HTML SPA, **not** an `.app`; exempt from build-app.sh/install.sh/auto-backup/icon standards) | `npm run build` → one self-contained `dist/index.html` (all JS/CSS + the embedded R-SICCI instrument JSON inlined via `vite-plugin-singlefile`); `npm run dev` for local dev. Dual-mode: *Take the survey* (administers R-SICCI Draft v0.1 — 11 modules, the 38-theme D matrix, Module-E selected-theme follow-ups, opt-in Module J, eligibility gate, DFI support screen, localStorage pause/resume; exports one data file) and *Score a data file* (researcher imports the file → descriptive profile). Data file is AES-GCM/PBKDF2-encrypted `.rsicci` by default or plain `.json`; no PII. Pure DOM-free scoring engine (`src/score/engine.ts`) = the 10 score rules + 9 axes; **reverse-coding follows the published score_rules, not the item-bank flag** (only CON_PRESSURE/WB_STRESS reversed; SDS sums SOC items raw); 97/98/99 = missing. **SRI/Module J is scored as an individual researcher-only descriptive index** (overall + thought/severity sub-scores + per-theme + prevalence; relaxed from the draft's aggregate-only stance per maintainer direction) — kept restricted, never participant-facing, never a risk/offending measure. Report also shows a per-theme breakdown of the 38 Module-D interest themes (raw A/D/P + interest mean/% + breadth flag), the non-restricted counterpart of the SRI per-theme table. **Hosted on GitHub Pages** (LIVE at bronty13.github.io/rsicci; `npm run deploy` → `scripts/deploy-pages.sh` builds + pushes the single-file artifact to the public `bronty13/rsicci` Pages repo, NFEditor pattern); fully client-side so no answers transmitted, but content is public + Pages logs visitor IPs. | `npm test` — vitest, 39 tests (engine incl. xlsx worked-example CII=50/CIB=2/CAP=40, missing-code/reverse/eligibility/SINGLE-coding; datafile encrypt-decrypt round-trip + wrong-pass; integration score→encrypt→reload→decrypt→re-score + headless `App`/`Report` render). `npm run typecheck` for `tsc --noEmit`. |
...
| `oldfiles/` (Python, single-file CLI) | `python3 oldfiles.py SOURCE --older-than 1y [...]` (or `./install.sh` → `oldfiles` on PATH). Lists files older than a threshold; **dry-run by default**, `--delete` → Trash (Send2Trash, lazily bootstrapped into a local `.venv`; verified cross-volume to `/Volumes/*/.Trashes` on REDONE), `--delete-permanent` → stdlib `os.remove`. The **manual** reclaim step after `pattic` ad-hoc B2 is verified (no automation by design). Protected-path guard; created-date default (`--by` to switch). | `python3 test_oldfiles.py` (23 tests, stdlib unittest) |
...
- **End-of-task default: build + install + launch — every time, no asking.**
  ...
  PurpleDedup, PurpleTracker, PurpleAttic, PurpleMirror, PurplePeek, Ircle, and any future siblings), finish every code-touching
