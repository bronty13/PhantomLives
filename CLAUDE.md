...
| `PurpleMark/` (Swift, SwiftUI macOS app — native Markdown editor + default `.md` handler + Finder Quick Look previewer, OpenMark-style) | `./build-app.sh` → `PurpleMark.app` in `/Applications/` (XcodeGen, Developer-ID-or-adhoc-signed, build + install + relaunch; `--no-install` / `--no-open` / `BUILD_ONLY=1` opt-outs). Four targets: app, `PurpleMarkRenderCore` framework (bundled offline markdown-it + Mermaid + KaTeX), `PurpleMarkQuickLook` `QLPreviewProvider` app-extension, tests. Single-pane Document⇄Markdown toggle; auto-runs the launch-time backup standard (scoped to prefs/recents — docs are user files). | `./run-tests.sh` — XCTest, 14 tests (outline parse, doc stats, standalone-HTML render + font inlining, backup zip/retention, HTML export, settings round-trip, file filtering). |
...
- **End-of-task default: build + install + launch — every time, no asking.**
  ...
  PurpleDedup, PurpleTracker, and any future siblings), finish every code-touching
