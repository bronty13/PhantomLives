...
| `Ircle/` (Swift, SwiftUI macOS app — a nostalgic recreation of the classic Mac *Ircle* IRC client) | `./build-app.sh` → `Ircle.app` in `/Applications/` (plain SwiftPM, depends on `../IRCKit`, Developer-ID-or-adhoc-signed, build + install + relaunch + freshness proof; code-generated icon; `--no-install` / `--no-open` / `BUILD_ONLY=1` opt-outs). Clean-room Platinum/Graphite single-window UI (Channelbar, message area, nick list, input line) + a separate Faces window (per-user avatars: assigned image or generated monogram) over an `IrcleSession` session layer on IRCKit. TLS/SASL/proxy; channels/queries/auto-join; slash commands; mIRC color/formatting rendering; auto-backup standard (settings + faces → `~/Downloads/Ircle backup/`). SwiftUI macOS-14 window sizing is unreliable — `AppDelegate` grows degenerate windows on `didBecomeKey`. Not a literal port (Ircle 3.x is closed-source) — see `Ircle/README.md`. | `./run-tests.sh` — swift-testing, 41 tests (backup, nick-list/user ordering/case-folding, session plumbing + inbound dispatch, mIRC renderer, faces store/graphics). |
