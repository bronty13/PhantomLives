...
| `IRCKit/` (Swift, SwiftPM **library** — the shared IRC wire engine) | `swift build`. No `.app` (it's a library). Pure Foundation + Network: the RFC 1459 / IRCv3 line parser (`IRCMessage`) + sanitizer/masker (`IRCSanitize`), the `NWConnection` transport (`IRCClient` + `IRCConnectionConfig`/`State`), CAP/SASL state machine (`SASLNegotiator`, internal), SOCKS5/HTTP-CONNECT framer (`ProxyFramer`, internal), the `IRCConnectionEvent` fan-out enum, `SASLMechanism`/`ProxyType`, and `IRCText.stripFormatting`. Consumed by **both Ircle and PurpleIRC** via `.package(path: "../IRCKit")`. Extracted from PurpleIRC's engine spine; engine types are `public`. (PurpleIRC migrated onto IRCKit in IRCKit Phase 2 — its old engine-file copies are deleted; `IRCConnection` is the only session layer left there.) | `./run-tests.sh` — swift-testing, 72 tests (parser, sanitizer/masker, SASL state machine, formatting stripper). |
| `Ircle/` (Swift, SwiftUI macOS app — a nostalgic recreation of the classic Mac *Ircle* IRC client) | `./build-app.sh` → `Ircle.app` in `/Applications/` (plain SwiftPM, depends on `../IRCKit`, Developer-ID-or-adhoc-signed, build + install + relaunch + freshness proof; code-generated icon; `--no-install` / `--no-open` / `BUILD_ONLY=1` opt-outs). Clean-room Platinum/Graphite single-window UI (Channelbar, message area, nick list, input line) + a separate Faces window (per-user avatars: assigned image or generated monogram) over an `IrcleSession` session layer on IRCKit. multi-server (one `IrcleSession` per network; Channelbar grouped by server; built-in common-network presets via `ServerProfile.defaultServers()`); TLS/SASL/proxy; channels/queries/auto-join; slash commands; mIRC color/formatting rendering; passwords in the **Keychain** (`SecretStore`/`KeychainSecretStore`, device-only; never in `settings.json`); AppleScript (`Ircle.sdef` + `@objc` `NSScriptCommand` subclasses: connect/join/say/current-nickname); Sparkle 2 auto-update (`Scripts/release.sh` → notarized + EdDSA-signed zip + `appcast.xml`; shared fleet key; `UpdaterController` no-ops on a placeholder key; see `RELEASING.md`); auto-backup standard (settings + faces → `~/Downloads/Ircle backup/`). SwiftUI macOS-14 window sizing is unreliable — `AppDelegate` grows degenerate windows on `didBecomeKey`. Not a literal port (Ircle 3.x is closed-source) — see `Ircle/README.md`. | `./run-tests.sh` — swift-testing, 59 tests (backup, nick-list/user ordering/case-folding, session plumbing + inbound dispatch, mIRC renderer, faces, multi-server model, presets, Keychain credentials). |
