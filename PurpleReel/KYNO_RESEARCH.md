...
| 5 | Parity gap | Metadata import/merge from FCPX + Premiere XML back into Kyno | Kyno docs | https://lesspain.software/kyno/integrations/ | Large | Shipped | Metadata menu → "Import FCPXML…". `FCPXMLImportService.importXML(at:db:)` parses 1.8-1.11 with a permissive `XMLParserDelegate` (handles `<asset-clip>` / `<clip>` / `<mc-clip>` / `<sync-clip>` containers, FCP rational times like `6006/30000s`, `<media-rep src="file://…"/>` percent-decoded paths). Matches NLE clips to catalogue assets by URL-decoded path → filename fallback. Merge is additive: markers de-duped by ±1/fps + note match; keywords become tags (case-insensitive); FCP `favorite` raises rating to 5★ but never demotes; `<metadata><md key=… value=…/></metadata>` fills empty Title/Description/Reel/Scene/Shot/Take/Angle/Camera fields only; `<note>` lands in description. Reports matched/added/skipped/unmatched with a "rescan first" tip when the editor referenced files PurpleReel doesn't know about. Round-trips cleanly with the existing FCPXMLWriter. (Sprint 4) |
...
| 7 | Parity gap | Shared cache for thumbnails/metadata on NAS/SAN (`.LP_Store/`) | Kyno docs | https://support.lesspain.software/support/solutions/articles/12000075313-volume-settings | Large | Shipped | Settings → General → "Workspace Cache (Shared NAS / SAN)" toggles `workspaceCacheEnabled`. When on, `WorkspaceCacheService.saveAsset(_:db:)` writes `<dir>/.purplereel/<filename>.json` after every mutation (rating, tags, markers, subclips, log fields, poster frame). MediaScanner's read path (`scan` + `scanShallow`) checks `loadIfFresh(for:)` first; matching modtime hydrates technical fields (codec/dims/fps/duration/audio/recorded/created/isVFR/sha1/posterFrame) without an AVAsset probe. After scan, `hydrateUserMetadataFromCache` runs the user portion through additive merge (rating raises only, tags union, log fields fill empty slots only, markers/subclips de-dupe by tc±1/fps + note). Local SQLite stays canonical; sidecars are advisory. Thumbnails and waveforms stay local — they regenerate cheaply and depend on hardware/colour state. Last-writer-wins on concurrent edits; real merge is a follow-up. (Sprint 4) |
| 8 | Parity gap | Combine multiple clips into one (assembly-cut without NLE) | Release notes | https://support.lesspain.software/support/solutions/articles/12000016005-release-notes-for-kyno | Large | Shipped | Convert → "Combine Clips…" (⌘⇧J). `CombineClipsJob.run()` builds an `AVMutableComposition`, inserts each source's video + audio tracks head-to-tail at a running CMTime cursor, copies the first clip's `preferredTransform` + `naturalSize` so portrait phone footage stays upright, and exports via `AVAssetExportSession` using a non-pass-through native preset (defaults to ProRes 422; H.264/HEVC/4444 available). `CombineClipsSheet` lists the current multi-selection in playback order, lets the user reorder with up/down arrows and remove rows, picks the preset + destination + filename, and live-tracks export progress. Reveals the output on success and kicks `appState.rescan()` so the new file shows up in the catalogue. Whole-clip MVP; per-clip in/out trim is a follow-up. (Sprint 4) |
...
| 29 | Parity gap | Spanned-clip detection (multi-file recordings auto-joined) | Kyno docs | https://lesspain.software/kyno/features/ | Large | Shipped | `SpanDetectionService.detect(in:)` runs a pure-Swift heuristic over the in-memory catalogue: same-directory + same-extension + same (codec / dims / fps / audioCodec) + sequential trailing-digit filename (`MVI_0001`/`MVI_0002`, `C0001`/`C0002`, `00000`/`00001`) + modtime within 120s. Conservative — favours missing a span over false-positive gluing. AppState exposes `spanGroups: [SpanGroup]`, recomputed at the end of every scan (rescan + scanOnDemand). New "Spanned Clips" sidebar section appears only when spans exist; each row → click reveals the segments, right-click → "Combine Segments…" opens the row-8 `CombineClipsSheet` pre-populated with the segments in order. (Sprint 4) |
...
| 57 | Competitive improvement | Centralized cross-volume search (no offline limit) | Review article | https://www.richardlackey.com/kyno-review-media-management-for-video-creators/ | Large | Shipped | v7 schema adds `asset.volumeUUID` (indexed) + `asset.volumeLabel`, captured by `MediaScanner.resolveVolume(forPath:)` via `URLResourceKey.volumeIdentifierKey` / `volumeLocalizedNameKey`. Catalogue rows persist across unmounts — PurpleReel was already non-destructive on scan; cells consult `AppState.onlinePaths` (re-snapshot via `FileManager.fileExists` after each scan + on every `volumeDid(Un)Mount`) and overlay a translucent black layer + `cloud.slash` badge + volume label when offline. New filter `.onlineStatus(.online / .offline)` in the Browse → Filter menu under "Volume / Online status". `VolumeWatcher.handleMounted` calls `AppState.reconnectVolume(uuid:newRoot:)` → `DatabaseService.updateAssetPathPrefix(...)` to re-anchor catalogue paths when macOS remounts a card under a renamed prefix (e.g. "MEDIA" → "MEDIA 1"). Lackey's "no offline search, no centralized database" gap closed. (Sprint 4) |
...
| 66 | Competitive improvement | Silverstack "workflow chains" (offload → transcode → upload → report as one job) | Competitor feature | https://pomfort.com/silverstackxt/ | Large | Shipped | New `WorkflowChain` model + `WorkflowChainsStore` (JSON in UserDefaults). Three step kinds wrap existing services: Verified Backup (`VerifiedBackupService` + chosen hash algo + MHL format + destinations), Transcode (`TranscodeQueue` enqueues every media file under source with a chosen preset; defaults to `<source>/Proxies/`), Export Report (`ReportExporter` HTML or CSV to `~/Downloads/PurpleReel/<chain>-<stamp>.<ext>`). `WorkflowChainRun` (@MainActor ObservableObject) executes steps sequentially with per-step queued/running/finished/failed/cancelled state; surfaces in the sheet as a checklist with live detail ("3/12 files verified", "8/12 transcoded"). New `WorkflowChainsSheet` does CRUD + step reorder (up/down) + run with progress + reveal-artifacts. File → "Workflow Chains…" (⌘⇧Y). Per-chain `runOnCameraMediaMount` flag drives auto-trigger: when a freshly-mounted volume looks like camera media (existing `looksLikeCameraMedia` heuristic — DCIM / AVCHD / PRIVATE / BPAV / XDROOT at root), `AppState.offerWorkflowChainOnMount(volumeURL:)` shows a Run-Chain / Skip alert; on Run, the sheet pops with the chain pre-selected, the source pre-populated, and the run started after a brief layout-settle defer. (Sprint 4) |
...
| 82 | Annoyance to solve | "Disconnect drive before importing metadata XML" workaround | Forum/Reddit post | https://support.lesspain.software/support/discussions/topics/12000027432 | Large | Shipped | Covered by row 5's `FCPXMLImportService` + row 80's `KynoImportService`. Both importers walk the chosen root and resolve assets by URL-decoded path → filename — no special drive state required. Neither cares whether the source volume is mounted at the same path as when the XML was written; row 57's volume-UUID-based reconnection makes path drift even less brittle. The bizarre Kyno workaround simply never applied to PurpleReel by construction. (Sprint 4) |
