...
struct PurpleReelApp: App {
    ...
    // Report export dispatch moved to `AppState.runReportExport(...)`
    // (C12). The menu items + right-click now route through
    // `appState.openReportDefinition(format:)` so the user sees
    // Kyno's section-selection dialog first.
    ...
    var body: some Scene {
        ...
        .commands {
            ...
            CommandGroup(after: .newItem) {
                ...
                Menu("Export Report") {
                    Button("CSV…") {
                        appState.openReportDefinition(format: .csv)
                    }
                    Button("HTML (with thumbnails)…") {
                        appState.openReportDefinition(format: .html)
                    }
                    Button("Excel (XLSX, with thumbnails)…") {
                        appState.openReportDefinition(format: .xlsx)
                    }
                }
