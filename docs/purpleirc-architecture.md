...
- **The wire engine lives in the shared `IRCKit` package** (`../IRCKit`, also used by Ircle) — `IRCClient`, `IRCMessage`/`IRCSanitize`, `IRCConnectionState`/`Config`, `IRCConnectionEvent`, `SASLMechanism`, `ProxyType`, `IRCText.stripFormatting`. PurpleIRC `import IRCKit`; its own engine-file copies were deleted in IRCKit Phase 2. `IRCConnection` (the session layer) and everything UI-side remain in PurpleIRC.
- **`IRCConnection`** — one per network. Owns an `IRCClient` (from IRCKit), buffers, reconnect state, and a per-connection event subject.
- **`IRCClient`** (IRCKit) — RFC 1459 parsing + `NWConnection` transport; SASL (PLAIN/EXTERNAL) and IRCv3 CAP negotiation live here. `ProxyFramer` plugs in at the bottom of the protocol stack.
- **Event fan-out** — every line / state change flows through the `Sendable` `IRCConnectionEvent` enum (in IRCKit). `ChatModel.events` merges all connections (UUID-tagged) and is what bots, watchlist, and the assistant subscribe to.
