#!/bin/bash
# Reinstall the freshly-built PurpleSpeak.app into /Applications/.
#
# Usage:
...
# A graceful quit can be blocked indefinitely by an unsaved prompt or hung run
# loop, after which a plain `open` re-focuses the STALE copy. So we force-kill,
# WAIT until the process is gone, launch a NEW instance with `open -n`, and
# PROVE the running process started after the new binary was written.
set -euo pipefail
