// Phase 7 fallback — generic per-platform checklist for any bundle
// type without a dedicated runner. Same UI we shipped in v0.14.0;
// kept as the safety net for forward compatibility (an unknown
// bundleType still gets a usable Post tab).
...
export function GenericRunner({ summary }: Props) {
  ...
  const refresh = useCallback(async () => {
    setError(null);
    try {
      setCards(await listBundlePostings(summary.uid));
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      >
        {cards.map((c) => (
          <PlatformCard key={c.target.id} card={c} bundle={summary} onChange={refresh} />
        ))}
      </div>
    </div>
  );
