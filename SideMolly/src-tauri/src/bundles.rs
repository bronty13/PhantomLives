...
pub fn ingest_bundle_inner<R: Runtime>(
    ...
    // post-bundle's artifacts/thumbnails payload and the SideMolly Summary
...
/// User-configurable settings for the SideMolly Summary PDF + the export-thumb
...
/// Global (not per-persona) default toggle states for the Edit tab's image +
/// video ops. The Edit tab seeds its checkboxes from these on open.
#[derive(Debug, Clone, Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct EditDefaults {
    pub image_watermark: bool,
    pub image_strip_exif: bool,
    pub image_rename: bool,
    pub video_watermark: bool,
    pub video_strip_metadata: bool,
    pub video_rename: bool,
}

#[tauri::command]
pub fn get_edit_defaults<R: Runtime>(
    handle: AppHandle<R>,
) -> Result<EditDefaults, BundleError> {
    let conn = open_conn(&handle)?;
    conn.query_row(
        "SELECT image_watermark, image_strip_exif, image_rename,
                video_watermark, video_strip_metadata, video_rename
           FROM edit_defaults WHERE id = 1",
        [],
        |r| Ok(EditDefaults {
            image_watermark: r.get::<_, i64>(0)? != 0,
            image_strip_exif: r.get::<_, i64>(1)? != 0,
            image_rename: r.get::<_, i64>(2)? != 0,
            video_watermark: r.get::<_, i64>(3)? != 0,
            video_strip_metadata: r.get::<_, i64>(4)? != 0,
            video_rename: r.get::<_, i64>(5)? != 0,
        }),
    )
    .map_err(Into::into)
}
...
#[tauri::command]
pub fn set_edit_defaults<R: Runtime>(
    handle: AppHandle<R>,
    settings: EditDefaults,
) -> Result<(), BundleError> {
    let conn = open_conn(&handle)?;
    conn.execute(
        "UPDATE edit_defaults SET
            image_watermark = ?1, image_strip_exif = ?2, image_rename = ?3,
            video_watermark = ?4, video_strip_metadata = ?5, video_rename = ?6,
            updated_at = datetime('now')
         WHERE id = 1",
        params![
            settings.image_watermark as i64,
            settings.image_strip_exif as i64,
            settings.image_rename as i64,
            settings.video_watermark as i64,
            settings.video_strip_metadata as i64,
            settings.video_rename as i64,
        ],
    )?;
    Ok(())
}
...
/// Conn-based helper for the SideMolly Summary PDF + post-bundle compose, which
...
pub fn get_processed_previews<R: Runtime>(
    ...
    // (rotation already baked in by process_image). For video outputs the
    // processed file is an .mp4 — base64-ing it as image/jpeg gives the
    // browser garbage and the row renders the 🖼 placeholder (the bug Robert
    // hit 2026-05-24). Fall back to the source video's
    // bundle_files.thumbnail_path (an ffmpeg-extracted frame at t=1s), but
    // rotate it by the file's rotation_degrees so the preview is upright.
    let mut stmt = conn.prepare(
        "SELECT bf.in_zip_path, bf.kind, pf.output_path, bf.thumbnail_path, bf.rotation_degrees
           FROM processed_files pf
           JOIN bundle_files bf ON bf.id = pf.bundle_file_id
          WHERE bf.bundle_uid = ?1
    ...
    let rows: Vec<(String, String, String, Option<String>, i64)> = stmt
        .query_map(params![uid], |row| Ok((
            row.get(0)?, row.get(1)?, row.get(2)?, row.get::<_, Option<String>>(3)?, row.get(4)?,
        )))?
        .collect::<rusqlite::Result<Vec<_>>>()?;
    drop(stmt);
    ...
    for (in_zip, kind, output_path, thumb_path, rotation) in rows {
        if out.contains_key(&in_zip) { continue; } // most-recent op wins
        // Pick the right source for the preview JPEG + whether it needs righting.
        let (preview_src, apply_rotation): (Option<String>, bool) = match kind.as_str() {
            "image" => (Some(output_path), false), // processed JPEG already rotated
            "video" => (thumb_path.filter(|p| !p.is_empty()), true),
            _ => (None, false),
        };
        let Some(src) = preview_src else { continue };
        let bytes = if apply_rotation {
            match crate::thumbnails::rotated_jpeg_bytes(Path::new(&src), rotation) {
                Some(b) => b,
                None => continue,
            }
        } else {
            let Ok(b) = fs::read(&src) else { continue };
            b
        };
...
mod tests {
    ...
    fn fresh_db() -> Connection {
        ...
        conn.execute_batch(include_str!("../migrations/023_edit_defaults.sql")).unwrap();
        conn
    }
