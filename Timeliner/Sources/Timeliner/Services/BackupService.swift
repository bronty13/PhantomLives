...
/// Auto-backup service for Timeliner.
///
/// PhantomLives convention: every app that owns persistent user data runs an
/// automatic backup on launch. Default location: `~/Downloads/Timeliner backup/`.
...
enum BackupService {
    ...
    /// Filename prefix for archives written by this service. The archive
    /// name format is `Timeliner-yyyy-MM-dd-HHmmss.zip`. Used by both the
    /// retention trim and the listing UI to recognize "our" archives.
    static let archivePrefix = "Timeliner-"
    ...
    static func runOnLaunchIfDue(settingsStore: SettingsStore) {
        ...
        // Synchronous on the main actor — backups are small (sqlite + JSON)
        // and the operation completes well under the visible UI delay budget.
        // Doing it inline guarantees a failed write surfaces via NSLog rather
        // than vanishing into a detached task.
        do {
            let url = try doBackup(settingsStore: settingsStore)
            NSLog("Timeliner: backup-on-launch wrote \(url.lastPathComponent)")
        } catch {
            NSLog("Timeliner: backup-on-launch failed — \(error.localizedDescription)")
        }
    }
    ...
    static func runBackup(supportDir: URL, backupDir: URL) throws -> URL {
        ...
        let tempZip = fm.temporaryDirectory
            .appendingPathComponent("timeliner-backup-\(UUID().uuidString).zip")
    ...
    enum RestoreError: Error, LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .missingDatabase:        return "Backup didn't contain timeliner.sqlite"
    ...
    /// contains a working `timeliner.sqlite`, count rows, return a summary
    ...
    static func verifyArchive(at archiveURL: URL) throws -> VerifyResult {
        ...
        let staging = fm.temporaryDirectory
            .appendingPathComponent("tl-verify-\(UUID().uuidString)", isDirectory: true)
        ...
        let dbCandidate = staging.appendingPathComponent("timeliner.sqlite")
        ...
        do {
            ...
            try pool.read { db in
                ...
                caseCount   = try Int.fetchOne(db, sql: "SELECT COUNT(*) FROM cases")  ?? 0
    ...
    static func restoreArchive(at archiveURL: URL, into supportDir: URL) throws {
        ...
        let staging = fm.temporaryDirectory
            .appendingPathComponent("tl-restore-\(UUID().uuidString)", isDirectory: true)
        try fm.createDirectory(at: staging, withIntermediateDirectories: true)
        defer { try? fm.removeItem(at: staging) }
