...
- **Transcribe**: off. When on, audio/video attachments are run through the local Whisper model (default `turbo`, configurable in **Messages Exporter → Settings…**). Sidecars `<attachment>.transcript.json` and `<attachment>.transcript.txt` land next to each AV file; raw-mode sidecars are hashed and logged. **Master kill switch** at **Settings → Transcription → Enable transcription** — flip it off if you never need transcripts. **Launch-time preflight** probes the dependency chain (transcribe.py, Python 3.10+, ffmpeg, venv, mlx, mlx-whisper, mlx-lm, truststore) and auto-opens a warm one-button setup wizard on failure. **Pre-run gate** re-probes right before each export when Transcribe is on — broken venv shows the wizard immediately instead of letting the CLI spam tracebacks across every attachment. The setup workflow is clean-slate every time (`rm -rf .venv` → `python -m venv` → `pip install`); never `--force-reinstall` on top of existing files (which was the cause of the partial-install corruption loop that bedevilled 1.0.268). Per-run failures surface in a yellow "Last run reported a problem" banner instead of hiding inside the live-output pane. `/opt/homebrew/bin` is auto-prepended to the child `PATH` so Finder-launched runs find `brew`/`ffmpeg` the same way Terminal-launched ones do.
