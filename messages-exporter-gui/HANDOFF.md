...
Last updated: 2026-05-12. Range-precision fix: GUI now emits `HH:MM:SS` to the CLI, the form gained a seconds field next to each picker, and a new **Range precision → Expand start by 60 seconds** setting (default on) handles Messages.app's display-time rounding. New `RangeResolver` helpers in `Model/ExportRequest.swift` carry the pure logic and are covered by a dedicated test suite. For a per-feature timeline read CHANGELOG; this file does not pin a version because the bundle version increments on every commit.
