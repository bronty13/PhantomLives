...
| 🎁 **Bundles** | Compose delivery packages for Robert. Three flavors planned — **Content** (live now: title + persona + text-or-audio description + 3+ drag-reorder categories + drag-reorder media + go-live date + special instructions), Custom + Fan Site (next release). Publishes as a deterministic, SHA-256-hashed two-layer ZIP to `~/Downloads/Molly bundles/` ready to drop into Slack. A pre-publish wizard walks every field with audio playback + image thumbnails + a click-to-jump checklist of validation issues. Content publishes also auto-upsert a row into Clips with status `Bundled` so the go-live date shows on the calendar. |
