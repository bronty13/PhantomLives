...
| 🗜️ **Squish** | Shrink an oversized clip (e.g. a 4+ GB 4K `.mov`) down to a Slack-uploadable size (under 1 GB) while keeping the most quality the budget allows. Pick a video → one button → progress bar → a `<name> (Squished).mov` lands in `~/Downloads/Molly/`. Reuses the bundled native ffmpeg: downscales to a Full-HD box (orientation-aware), re-encodes H.264 (CRF 20) + AAC with a budget-derived `-maxrate` so it fills the byte budget without truncating; HDR (iPhone 4K) is tone-mapped to SDR. 100% local. |
...
| 1.28 | 🛠️ **Teaser MP4 now plays on Windows** — `MediaRecorder` wrote the MP4's `moov`/duration at the *end* (streaming layout) so Windows' native players rejected it as corrupt (fine on macOS, which fell back to `.webm`). Now encodes H.264 + AAC via **WebCodecs** and muxes a progressive, seekable `.mp4` (`moov` up front, real duration) with **`mp4-muxer`** (`fastStart: 'in-memory'`). MediaRecorder remains the WebKit fallback; no-AAC systems get a clean silent MP4. New dep: `mp4-muxer`. |
| 1.29 | 🎬 **GIF Studio runs on a bundled native ffmpeg** — replaced the WebView pipeline (canvas + gifenc + WebCodecs/mp4-muxer + ffmpeg-WASM) with a native ffmpeg engine in Rust (`src-tauri/src/media/`). Decodes **any** iPhone/Windows format (HEVC, H.264, **Dolby Vision HDR** with zscale tone-map, ProRes) for GIF / teaser MP4 / frame, input-seeking the original so only the trim is processed. ffmpeg/ffprobe GPL static builds are CI-bundled (~+80–100 MB); discovery = bundled → Settings override → PATH. Fixes "iPhone HEVC won't decode on Windows" + ffmpeg-WASM slowness. Removed `gifenc`/`mp4-muxer`/`@ffmpeg/*`. See `THIRD_PARTY_LICENSES.md`. |
