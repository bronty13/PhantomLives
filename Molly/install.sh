...
# Replaces /Applications/Molly.app with the freshly-built bundle and
# relaunches it so TCC entitlements stay anchored to a stable cdhash.
# Pure CLI / Windows installs do not use this — the Tauri .msi/.exe
# installer is the Windows path.
#
# Usage:
...
LOCAL_APP="$PROJECT_DIR/src-tauri/target/release/bundle/macos/Molly.app"
INSTALL_PATH="/Applications/Molly.app"
...
osascript -e 'tell application "Molly" to quit' >/dev/null 2>&1 || true
sleep 1
...
echo "✅ Molly installed to $INSTALL_PATH"
