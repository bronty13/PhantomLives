...
- **📣 Platforms** — your social platforms (Reddit, X, Instagram, TikTok…) — the very ones the 🪙 Social piggy bank tracks. Add, edit, or delete any of them, each with a name, short code, emoji, color, and sort order. ✨ **New:** you can now set each one's **daily goal 🪙** right here — how many posts a day fills its piggy bank. Bump TikTok up to 3/day when you're feeling unstoppable, or set a platform to **0 = paused** for the month and the streak will sweetly skip it (no deletion, just a little nap 🌸). This is the *only* place to tweak Reddit's goal, since Reddit has its own deep tab instead of a piggy-bank tab. Each row shows its goal as a `🪙 N/day` badge (or `⏸️ paused`) so you can see everything at a glance.
...
- **🔒 Make private** — a little checkbox right where the go-live date usually sits, and it starts **checked** (that's the default — you can flip the starting state in **Settings → Bundler → YouTube bundle defaults**). When it's checked, the video goes up *private* and goes live the **moment you publish** — so there's nothing to schedule, and the date picker politely disappears. 💕 Uncheck it and the **Go-live date** comes right back: required, no dates in the past, with my gentle *"are you allowing enough time for editing?"* nudge if it's today or within 5 days. 🌷 (Switching back to private clears any date you'd set — no leftovers.)
- **Special instructions** — optional notes for Robert.
- **Also Post SFW ManyVids — Yes / No** — at the very bottom. Tap **Yes** if you'd also like Robert to post a SFW cut to ManyVids; **No** otherwise (that's the default, and you can change the default in the same Settings card). Either way it rides along in the bundle's notes so Robert sees your choice. ✨
...
- For **Content / Custom / ▶️ YouTube** bundles: a **Clip writeback** summary like "5 of 5 files matched." Molly tries to match each posted file back to a row in your Clips list by filename — when she finds a match she logs the posting against that clip. If a file's name doesn't match any clip, she still records the posting (just without a clip link). *(YouTube bundles import exactly like Content ones — no scary "type mismatch" note, I promise! 💕)*
...
## 🗜️ Squish — make a big clip small enough to upload 💕
...
So I made you a fix! Find **🗜️ Squish** in the sidebar. It's the easiest thing in the whole app:
...
3. Tap **🗜️ Squish it under 1 GB!** — and that's it! You're done. Go sip your coffee ☕.
...
When I'm finished I'll show you the happy news — *"Squished! 4.2 GB → 0.9 GB 🗜️"* — and your new, slimmer clip is waiting in your **Downloads → Molly** folder, named like `YourClip (Squished).mov`. Tap **📂 Show in folder** to pop right to it, then drag it into Slack. 🩷
