...
- **Sales** — click **🛒 + Add sale** to record a transaction. Pick the product, enter quantity (e.g. `10` for 10 minutes), the unit price defaults from the product but you can override it, and the total auto-computes; editing the total back-solves the unit price for line-level discounts. Add notes describing what was sold. Sales **are editable and deletable** via the Edit / Delete buttons on each row. The customer's lifetime sales total appears as a `💖 $X.XX` pill in the editor header (hidden when zero). Sales also automatically appear in **Income → Adhoc income** (marked 🛒) and roll into the MTD/YTD totals on Reports and Home — they're treated as one-off income, just sourced from a specific customer.
