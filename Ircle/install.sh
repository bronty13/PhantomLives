#!/bin/bash
# Reinstall the freshly-built Ircle.app into /Applications/.
#
# Usage:
...
# A graceful `osascript ... quit` can be blocked indefinitely by a hung run
# loop. When that happens the old process keeps running and a plain `open` just
# re-focuses the STALE copy — the new binary on disk never runs. So we
# force-kill, *wait until the process is actually gone*, launch a NEW instance
# with `open -n`, and then PROVE the running process started after the new
# binary was written. Any failure is fatal and loud.
set -euo pipefail
...
# unrelated process that merely has "Ircle" in its argv.
...
echo "Terminating any running $APP (force)..."
