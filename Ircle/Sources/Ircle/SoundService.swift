...
final class SoundService {
    ...
    /// Play the named clip if it exists. The *decision* to play (CTCP vs
    /// per-event, and whether enabled) is the caller's; this just plays.
    /// No-ops in non-app contexts (e.g. unit tests).
    func play(name: String) {
        guard Bundle.main.bundleIdentifier != nil, let url = soundURL(for: name) else { return }
